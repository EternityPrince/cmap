...
static void
cmaper_scan_process_buffer_init(cmaper_scan_process_buffer_t *buffer) {
  if (buffer == NULL) {
    return;
  }
...
static void
cmaper_scan_process_buffer_dispose(cmaper_scan_process_buffer_t *buffer) {
  if (buffer == NULL) {
    return;
  }
...
static cmaper_err_t
cmaper_scan_process_buffer_reserve(cmaper_scan_process_buffer_t *buffer,
                                   size_t needed) {
...
static cmaper_err_t
cmaper_scan_process_buffer_append(cmaper_scan_process_buffer_t *buffer,
...
static cmaper_err_t
cmaper_scan_process_read_pipe(int *fd, cmaper_scan_process_buffer_t *buffer) {
  char chunk[4096];
  ...
  while (true) {
    ...
    if (read_size > 0) {
      cmaper_err_t rc =
          cmaper_scan_process_buffer_append(buffer, chunk, (size_t)read_size);
      if (rc != CMAPER_OK) {
        return rc;
      }
      continue;
...
static cmaper_err_t cmaper_scan_process_wait_loop(
    pid_t child_pid, int *stdout_fd, int *stderr_fd, int heartbeat_seconds,
    const char *heartbeat_label, int hard_timeout_seconds,
    cmaper_logger_t *logger, cmaper_scan_process_buffer_t *stdout_buffer,
    cmaper_scan_process_buffer_t *stderr_buffer, int *wait_status) {
  ...
  while (*stdout_fd >= 0 || *stderr_fd >= 0 || child_running) {
    ...
    if (child_running) {
      ...
      if (hard_timeout_seconds > 0 &&
          (now_ms - heartbeat.started_ms) >=
              ((long long)hard_timeout_seconds * 1000LL)) {
        if (logger != NULL) {
          const char *label =
              heartbeat_label != NULL ? heartbeat_label : "scan/process";
          cmaper_log(logger, CMAPER_LOG_WARN,
                     "%s: hard timeout reached (%ds), terminating process",
                     label, hard_timeout_seconds);
        }
      ...
      if (heartbeat_seconds > 0) {
        ...
        if (cmaper_scan_heartbeat_should_emit(&heartbeat, now_ms,
                                              &elapsed_ms)) {
          const char *label =
              heartbeat_label != NULL ? heartbeat_label : "scan/process";
          cmaper_log(logger, CMAPER_LOG_WAIT, "%s: still running after %llds",
                     label, elapsed_ms / 1000LL);
        }
      }
    }
...
static void cmaper_scan_process_ensure_waited(pid_t child_pid,
                                              int *wait_status) {
...
cmaper_scan_process_run(const cmaper_scan_process_request_t *request,
  ...
  if (request->program_path == NULL || request->program_path[0] == '\0' ||
      request->argv == NULL) {
    return CMAPER_ERR_INVALID_ARGUMENT;
  }
  ...
  rc = cmaper_scan_process_wait_loop(
      child_pid, &stdout_pipe[0], &stderr_pipe[0], request->heartbeat_seconds,
      request->heartbeat_label, request->hard_timeout_seconds, logger,
      &stdout_buffer, &stderr_buffer, &wait_status);
  if (rc != CMAPER_OK) {
