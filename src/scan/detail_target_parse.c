...
cmaper_err_t cmaper_scan_detail_target_extract_probe_ports(
  ...
  if (probe_xml_path == NULL || target_ip == NULL || out_ports == NULL ||
      out_port_count == NULL) {
  ...
  *out_port_count = 0;
  ...
  for (i = 0; i < document.host_count; ++i) {
    ...
    if (host->status.state != NULL && strcmp(host->status.state, "up") == 0 &&
        fallback_up_host == NULL) {
      fallback_up_host = host;
    }

    if (ip != NULL && strcmp(ip, target_ip) == 0) {
      rc = cmaper_nmap_host_open_tcp_ports_sorted(host, out_ports,
                                                  out_port_count);
      cmaper_nmap_xml_document_dispose(&document);
      return rc;
    }
  ...
  if (fallback_up_host != NULL) {
    rc = cmaper_nmap_host_open_tcp_ports_sorted(fallback_up_host, out_ports,
                                                out_port_count);
    cmaper_nmap_xml_document_dispose(&document);
    return rc;
  }
