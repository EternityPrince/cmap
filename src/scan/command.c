...
static cmaper_err_t
cmaper_scan_command_push_arg(cmaper_scan_command_t *command, const char *value,
  ...
  if (command->argc >= CMAPER_SCAN_COMMAND_MAX_ARGS) {
    cmaper_scan_command_diag_setf(
        diag, "argv", "nmap command exceeded internal argument limit (%d)",
        CMAPER_SCAN_COMMAND_MAX_ARGS);
    return CMAPER_ERR_IO;
  }

  written = snprintf(command->arg_data[command->argc],
                     sizeof(command->arg_data[command->argc]), "%s", value);
...
static void
cmaper_scan_command_finalize_render(cmaper_scan_command_t *command) {
  size_t i;
  size_t offset = 0;
...
cmaper_scan_discovery_plan_build(const cmaper_scan_plan_t *scan_plan,
  ...
  discovery_plan->top_ports =
      cmaper_scan_discovery_default_top_ports(scan_plan->profile);
  discovery_plan->timing_template = scan_plan->timing_template;
  discovery_plan->spoof_requested =
      scan_plan->spoof_mac_mode != CMAPER_SCAN_SPOOF_MAC_OFF;

  if (snprintf(discovery_plan->representative_target,
               sizeof(discovery_plan->representative_target), "%s",
               identity->representative_target) >=
      (int)sizeof(discovery_plan->representative_target)) {
    cmaper_scan_command_diag_setf(
        diag, "target", "representative target exceeds internal buffer limit");
    return CMAPER_ERR_IO;
  }

  if (discovery_plan->target_is_cidr && !discovery_plan->no_ping &&
...
cmaper_err_t cmaper_scan_command_build_discovery(
  ...
  rc = cmaper_scan_command_push_arg(
      command, paths->nmap_bin[0] != '\0' ? paths->nmap_bin : "nmap", diag);
  if (rc != CMAPER_OK) {
    return rc;
  }
  ...
  if (discovery_plan->kind == CMAPER_DISCOVERY_KIND_HOST_DISCOVERY) {
    ...
    rc = cmaper_scan_command_push_arg(
        command,
        discovery_plan->transport == CMAPER_DISCOVERY_TRANSPORT_SYN ? "-sS"
                                                                    : "-sT",
        diag);
    if (rc != CMAPER_OK) {
      return rc;
    ...
    if (discovery_plan->use_exact_ports) {
      ...
      rc = cmaper_scan_command_push_arg(command, discovery_plan->exact_ports,
                                        diag);
      if (rc != CMAPER_OK) {
        return rc;
      }
      ...
      snprintf(top_ports_arg, sizeof(top_ports_arg), "%d",
               discovery_plan->top_ports);
  ...
  snprintf(timing_arg, sizeof(timing_arg), "-T%d",
           discovery_plan->timing_template);
  ...
  if (discovery_plan->spoof_applied) {
    ...
    rc = cmaper_scan_command_push_arg(command, discovery_plan->spoof_value,
                                      diag);
    if (rc != CMAPER_OK) {
      return rc;
    }
  ...
  rc = cmaper_scan_command_push_arg(command, discovery_plan->target_expression,
                                    diag);
  if (rc != CMAPER_OK) {
    return rc;
  }
