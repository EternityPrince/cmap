...
static void cmaper_scan_sanitize_component(
    const char *input,
    char *output,
    size_t output_cap
) {
...
static cmaper_err_t cmaper_scan_artifact_join_path(
    char *out,
    size_t out_cap,
    const char *left,
    const char *right
...
static cmaper_err_t cmaper_scan_artifact_resolve_session_xml_dir(
    ...
    if (paths == NULL || policy == NULL || out_xml_dir == NULL || out_xml_dir_cap == 0) {
        return CMAPER_ERR_INVALID_ARGUMENT;
    }
    ...
    rc = cmaper_fs_parent_directory(paths->xml_output_dir, output_root, sizeof(output_root));
    if (rc != CMAPER_OK) {
        return rc;
    }

    rc = cmaper_scan_artifact_join_path(
        session_dir,
        sizeof(session_dir),
        output_root,
        policy->session_id
    ...
    rc = cmaper_scan_artifact_join_path(out_xml_dir, out_xml_dir_cap, session_dir, "xml");
    if (rc != CMAPER_OK) {
        return rc;
    }
