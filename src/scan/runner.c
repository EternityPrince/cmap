...
static cmaper_err_t cmaper_scan_make_temp_xml_path(
    ...
    if (paths == NULL || prefix == NULL || prefix[0] == '\0' || out_path == NULL || out_path_cap == 0) {
    ...
    if (cmaper_fs_ensure_directory_recursive(paths->xml_output_dir) != CMAPER_OK) {
...
static cmaper_err_t cmaper_scan_command_set_xml_output(
    ...
    if (output_index != (size_t) -1 && output_index < (size_t) command->argc) {
        written = snprintf(
            command->arg_data[output_index],
            sizeof(command->arg_data[output_index]),
            "%s",
            output_path
        );
        if (written < 0 || written >= (int) sizeof(command->arg_data[output_index])) {
            cmaper_scan_command_diag_setf(diag, "argv", "xml output path is too long");
            return CMAPER_ERR_IO;
        }
        ...
        for (i = 0; i < (size_t) command->argc; ++i) {
            ...
            render_written = snprintf(
                command->rendered + used,
                sizeof(command->rendered) - used,
                "%s%s",
                prefix,
                command->argv[i]
            );
            if (render_written < 0 || (size_t) render_written >= (sizeof(command->rendered) - used)) {
    ...
    cmaper_scan_command_diag_setf(diag, "argv", "failed to locate -oX argument in command");
...
cmaper_err_t cmaper_scan_runner_run(
    ...
    rc = cmaper_scan_make_temp_xml_path(
        &runtime->paths,
        "discovery",
    ...
    if (result->discovery_plan.kind == CMAPER_DISCOVERY_KIND_HOST_DISCOVERY
        ...
        cmaper_log(
            &runtime->logger,
            CMAPER_LOG_WARN,
            "scan/discovery: host-discovery may miss silent hosts; use --no-ping for full coverage"
        );
    }
    ...
    if (!cmaper_fs_path_exists(discovery_temp_xml_path)) {
        cmaper_log(&runtime->logger, CMAPER_LOG_FAIL, "scan/discovery: nmap did not produce discovery xml");
        rc = CMAPER_ERR_PARSE;
        goto cleanup;
    }
    discovery_temp_xml_exists = true;
    result->discovery_xml_size = cmaper_scan_file_size_or_zero(discovery_temp_xml_path);
    ...
    rc = cmaper_nmap_xml_parse_file(discovery_temp_xml_path, &discovery_document, &xml_diag);
    if (rc != CMAPER_OK) {
        cmaper_log(
            &runtime->logger,
            CMAPER_LOG_FAIL,
