...
static const char *
cmaper_scan_spoof_mode_name(cmaper_scan_spoof_mac_mode_t mode) {
  switch (mode) {
...
static void cmaper_scan_plan_apply_toggle(bool *target,
...
static void
cmaper_scan_plan_apply_overrides(cmaper_scan_plan_t *plan,
  ...
  cmaper_scan_plan_apply_toggle(&plan->service_detection,
  ...
  if (options->spoof_mac == CMAPER_SCAN_TOGGLE_DISABLE) {
    ...
    if (options->spoof_mac_value == NULL ||
        strcmp(options->spoof_mac_value, "random") == 0) {
...
void cmaper_scan_plan_diag_setf(cmaper_scan_plan_diag_t *diag,
...
void cmaper_scan_plan_apply_profile_policy(cmaper_scan_plan_t *plan,
...
cmaper_err_t cmaper_scan_plan_validate(const cmaper_scan_plan_t *plan,
  ...
  if (plan->profile == CMAPER_SCAN_PROFILE_UNSET) {
    cmaper_scan_plan_diag_setf(diag, "--profile",
                               "scan profile must be resolved");
    return CMAPER_ERR_CLI_USAGE;
  }

  if (plan->timing_template < 0 || plan->timing_template > 5) {
    cmaper_scan_plan_diag_setf(
        diag, "--timing", "timing template must be in range 0..5 (got: %d)",
  ...
  if (plan->exact_ports != NULL && plan->all_ports) {
    cmaper_scan_plan_diag_setf(
        diag, "--all-ports",
        "options '--all-ports' and '--ports/--exact-ports' cannot be used "
        "together");
    return CMAPER_ERR_CLI_USAGE;
  }
  ...
  if (plan->spoof_mac_mode == CMAPER_SCAN_SPOOF_MAC_CUSTOM &&
    ...
    cmaper_scan_plan_diag_setf(
        diag, "--spoof-mac", "custom spoofing mode requires a non-empty value");
    return CMAPER_ERR_CLI_USAGE;
  }
...
void cmaper_scan_plan_compute_capabilities(cmaper_scan_plan_t *plan) {
  ...
  plan->capabilities.privileged_required =
      plan->sudo || spoofing || plan->os_detection || plan->traceroute ||
      plan->udp_enrichment;
}

cmaper_err_t cmaper_scan_plan_normalize(cmaper_scan_plan_t *plan,
                                        const cmaper_scan_options_t *options,
                                        cmaper_scan_plan_diag_t *diag) {
  cmaper_err_t rc;
...
void cmaper_scan_plan_render_summary(FILE *stream,
  ...
  fprintf(stream,
          ...
          target, cmaper_scan_profile_name(plan->profile), ports,
          cmaper_scan_bool_word(plan->all_ports),
          cmaper_scan_bool_word(plan->no_ping), plan->timing_template,
          plan->detail_workers, cmaper_scan_bool_word(plan->service_detection),
          cmaper_scan_bool_word(plan->os_detection),
          cmaper_scan_bool_word(plan->sudo),
          cmaper_scan_spoof_mode_name(plan->spoof_mac_mode), spoof_value,
