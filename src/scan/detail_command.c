...
static void
cmaper_scan_detail_command_init(cmaper_scan_detail_command_t *command) {
  size_t i;
...
static cmaper_err_t
cmaper_scan_detail_command_push(cmaper_scan_detail_command_t *command,
                                const char *value) {
  int written;
  ...
  written = snprintf(command->args[command->argc],
                     sizeof(command->args[command->argc]), "%s", value);
...
static void
cmaper_scan_detail_command_finalize(cmaper_scan_detail_command_t *command) {
  size_t i;
  size_t offset = 0;
  ...
  for (i = 0; i < (size_t)command->argc; ++i) {
    ...
    written =
        snprintf(command->rendered + offset, sizeof(command->rendered) - offset,
                 "%s%s", prefix, command->argv[i]);
    if (written < 0 ||
        (size_t)written >= (sizeof(command->rendered) - offset)) {
...
static cmaper_err_t cmaper_scan_detail_ports_to_csv(const int *ports,
                                                    size_t port_count,
  ...
  for (i = 0; i < port_count; ++i) {
    int written = snprintf(out + offset, out_cap - offset, "%s%d",
                           i == 0 ? "" : ",", ports[i]);
...
void cmaper_scan_detail_spoof_policy_resolve(
  ...
  if (source_identity->has_spoofed_mac) {
    snprintf(policy->value, sizeof(policy->value), "%s",
             source_identity->spoofed_mac);
  }
}

static cmaper_err_t
cmaper_scan_detail_command_add_base(const cmaper_scan_detail_request_t *request,
...
static cmaper_err_t cmaper_scan_detail_command_add_transport(
    const cmaper_scan_plan_t *plan, cmaper_scan_detail_command_t *command) {
...
static cmaper_err_t
cmaper_scan_detail_command_add_timing(int timing_template,
                                      cmaper_scan_detail_command_t *command) {
  char timing_arg[16];
...
static int
cmaper_scan_detail_probe_timeout_seconds(cmaper_scan_profile_t profile) {
  switch (profile) {
...
static int
cmaper_scan_detail_enrichment_timeout_seconds(cmaper_scan_profile_t profile) {
  switch (profile) {
  case CMAPER_SCAN_PROFILE_LOW:
    return 180;
  case CMAPER_SCAN_PROFILE_MID:
...
static cmaper_err_t cmaper_scan_detail_command_add_host_timeout(
    int timeout_seconds, cmaper_scan_detail_command_t *command) {
  cmaper_err_t rc;
...
static cmaper_err_t cmaper_scan_detail_command_add_max_retries(
    int max_retries, cmaper_scan_detail_command_t *command) {
  cmaper_err_t rc;
...
static cmaper_err_t cmaper_scan_detail_command_add_script_pipeline(
  ...
  if (script_set->script_expression == NULL ||
      script_set->script_expression[0] == '\0') {
  ...
  return cmaper_scan_detail_command_push(command,
                                         script_set->script_expression);
}

cmaper_err_t cmaper_scan_detail_build_probe_command(
    const cmaper_scan_detail_request_t *request,
    const cmaper_scan_detail_target_t *target, const char *xml_output_path,
    const cmaper_scan_detail_spoof_policy_t *spoof_policy,
    cmaper_scan_detail_command_t *command) {
  return cmaper_scan_detail_build_probe_command_with_transport(
      request, target, CMAPER_SCAN_DETAIL_PROBE_TRANSPORT_DEFAULT,
      xml_output_path, spoof_policy, command);
}

cmaper_err_t cmaper_scan_detail_build_probe_command_with_transport(
    const cmaper_scan_detail_request_t *request,
    const cmaper_scan_detail_target_t *target,
    cmaper_scan_detail_probe_transport_t transport, const char *xml_output_path,
    const cmaper_scan_detail_spoof_policy_t *spoof_policy,
    cmaper_scan_detail_command_t *command) {
  cmaper_err_t rc;
  int probe_timing;
  ...
  rc = cmaper_scan_detail_command_add_probe_transport(request->plan, transport,
                                                      command);
  if (rc != CMAPER_OK) {
    return rc;
  }
  ...
  if (request->plan->all_ports) {
    ...
    probe_top_ports =
        cmaper_scan_discovery_default_top_ports(request->plan->profile);
    /* Keep probe broad enough to avoid false "no-open-ports" on common
     * services. */
    if (request->plan->profile == CMAPER_SCAN_PROFILE_HIGH &&
        probe_top_ports < 3000) {
      probe_top_ports = 3000;
    } else if (request->plan->profile == CMAPER_SCAN_PROFILE_MID &&
               probe_top_ports < 1000) {
      probe_top_ports = 1000;
    } else if (request->plan->profile == CMAPER_SCAN_PROFILE_LOW &&
               probe_top_ports < 300) {
      probe_top_ports = 300;
    }
  ...
  /* Bound per-host probe runtime to avoid practically unbounded scans on
   * filtered networks. */
  rc = cmaper_scan_detail_command_add_max_retries(
      cmaper_scan_detail_probe_max_retries(request->plan->profile), command);
  if (rc != CMAPER_OK) {
    return rc;
  }
...
cmaper_err_t cmaper_scan_detail_build_enrichment_like_command(
    ...
    const int *ports, size_t port_count, const char *xml_output_path,
    const cmaper_scan_detail_spoof_policy_t *spoof_policy,
    cmaper_scan_detail_command_t *command) {
  cmaper_err_t rc;
  char ports_csv[512];
  ...
  rc = cmaper_scan_detail_ports_to_csv(ports, port_count, ports_csv,
                                       sizeof(ports_csv));
  if (rc != CMAPER_OK) {
    return rc;
  }
  ...
  rc = cmaper_scan_detail_command_add_timing(request->plan->timing_template,
                                             command);
  if (rc != CMAPER_OK) {
    return rc;
  }
