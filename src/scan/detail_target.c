...
static void cmaper_scan_detail_sanitize_component(
    const char *input,
    char *output,
    size_t output_cap
) {
...
static cmaper_err_t cmaper_scan_detail_make_temp_xml_path(
    ...
    if (request == NULL || request->paths == NULL || ip == NULL || phase == NULL
        || out_path == NULL || out_path_cap == 0U) {
    ...
    if (cmaper_fs_ensure_directory_recursive(request->paths->xml_output_dir) != CMAPER_OK) {
    ...
    written = snprintf(
        out_path,
        out_path_cap,
        "%s/.tmp-detail-%zu-%s-%s.xml",
        request->paths->xml_output_dir,
        target_index,
        phase,
        ip_component
    );
...
static cmaper_err_t cmaper_scan_detail_save_final_xml(
    ...
    if (request == NULL || ip == NULL || source_path == NULL || host_result == NULL) {
        return CMAPER_ERR_INVALID_ARGUMENT;
    }
...
static int cmaper_scan_detail_port_compare(const void *left, const void *right) {
...
static cmaper_err_t cmaper_scan_detail_merge_open_tcp_ports(
    ...
    size_t base_port_count,
    const int *extra_ports,
    size_t extra_port_count,
    int **out_ports,
    ...
    *out_port_count = 0;
    ...
    if ((base_port_count > 0 && base_ports == NULL) || (extra_port_count > 0 && extra_ports == NULL)) {
        return CMAPER_ERR_INVALID_ARGUMENT;
    }
...
void cmaper_scan_detail_execute_for_target(
    ...
    if (target->has_open_tcp_ports
        ...
        && (request->plan->all_ports
            || (request->plan->exact_ports != NULL && request->plan->exact_ports[0] != '\0'))) {
        ...
        if (rc != CMAPER_OK) {
            cmaper_scan_detail_host_message_setf(host_result, "failed to prepare direct xml path");
            cmaper_scan_detail_progress_mark_stage(
                progress_state,
                target_index,
        ...
        if (rc != CMAPER_OK) {
            cmaper_scan_detail_host_message_setf(host_result, "failed to prepare probe xml path");
            cmaper_scan_detail_progress_mark_stage(
                progress_state,
                target_index,
                CMAPER_SCAN_DETAIL_STAGE_FAILED
        ...
        rc = cmaper_scan_detail_build_probe_command(
            request,
            target,
            probe_xml_path,
            &spoof_policy,
        ...
        if (rc == CMAPER_OK
            ...
            if (fallback_rc == CMAPER_OK && connect_probe_xml_path[0] != '\0') {
                ...
                if (fallback_rc == CMAPER_OK) {
                    snprintf(final_xml_path, sizeof(final_xml_path), "%s", connect_probe_xml_path);
        ...
        if (rc == CMAPER_OK
            ...
            if (fallback_rc == CMAPER_OK) {
                ...
                if (fallback_rc == CMAPER_OK) {
                    snprintf(final_xml_path, sizeof(final_xml_path), "%s", all_probe_xml_path);
                }
            }

            if (fallback_rc == CMAPER_OK
                && all_probe_port_count == 0
                && request->plan->sudo) {
                ...
                fallback_rc = cmaper_scan_detail_make_temp_xml_path(
                    ...
                    sizeof(all_connect_probe_xml_path)
                );
                ...
                if (fallback_rc == CMAPER_OK) {
                    if (progress_state == NULL || !progress_state->dynamic_render) {
                        cmaper_log(
                            request->logger,
                            CMAPER_LOG_INFO,
                            "scan/detail[%s]: probe fallback command (full tcp range -sT) => %s",
                            target->ip,
                            command.rendered
                        );
                    }
                ...
                if (fallback_rc == CMAPER_OK) {
                    ...
                    if (fallback_rc == CMAPER_OK) {
                        ...
                        snprintf(final_xml_path, sizeof(final_xml_path), "%s", all_connect_probe_xml_path);
                    }
                }
        ...
        if (rc != CMAPER_OK) {
            ...
            if (rc == CMAPER_OK && enrichment_xml_path[0] != '\0') {
                ...
                snprintf(final_xml_path, sizeof(final_xml_path), "%s", enrichment_xml_path);
            } else {
                host_result->used_probe_xml_as_final = true;
                host_result->success = true;
                cmaper_scan_detail_host_message_setf(
