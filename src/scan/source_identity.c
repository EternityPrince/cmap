...
static size_t cmaper_scan_copy_token(char *out, size_t out_cap,
                                     const char *value) {
  size_t i = 0;
...
static bool cmaper_scan_is_loopback_target(const char *target) {
  ...
  if (strcmp(target, "localhost") == 0 || strcmp(target, "::1") == 0 ||
...
static cmaper_err_t cmaper_scan_random_mac_generate(char *out, size_t out_cap) {
  ...
  if (snprintf(out, out_cap, "%02X:%02X:%02X:%02X:%02X:%02X", bytes[0],
...
cmaper_err_t
cmaper_scan_source_identity_resolve(const cmaper_scan_plan_t *plan,
  ...
  cmaper_scan_copy_token(identity->representative_target,
                         sizeof(identity->representative_target), plan->target);

  identity->representative_is_cidr =
      strchr(identity->representative_target, '/') != NULL;
  ...
  if (plan->spoof_mac_mode == CMAPER_SCAN_SPOOF_MAC_RANDOM) {
    cmaper_err_t rc = cmaper_scan_random_mac_generate(
        identity->spoofed_mac, sizeof(identity->spoofed_mac));
    if (rc != CMAPER_OK) {
      return rc;
    }
    ...
    identity->spoof_is_random = false;
    identity->has_spoofed_mac = true;
    snprintf(identity->spoofed_mac, sizeof(identity->spoofed_mac), "%s",
             plan->spoof_mac_value);
  }
...
const char *
cmaper_scan_source_identity_mode_name(cmaper_scan_source_identity_mode_t mode) {
  switch (mode) {
