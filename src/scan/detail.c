...
static void *cmaper_scan_detail_worker_entry(void *arg) {
    ...
    while (true) {
        ...
        cmaper_scan_detail_execute_for_target(
            pool->request,
            target,
            index,
            host_result,
...
cmaper_err_t cmaper_scan_detail_execute(
    ...
    if (request->logger->level <= CMAPER_LOG_INFO
        ...
        progress_state.hosts = (cmaper_scan_detail_host_progress_t *) calloc(
            progress_state.host_count,
            sizeof(cmaper_scan_detail_host_progress_t)
        );
        if (progress_state.hosts != NULL && pthread_mutex_init(&progress_state.lock, NULL) == 0) {
            progress_state.lock_initialized = true;
            progress_state.dynamic_render = true;
            for (i = 0; i < progress_state.host_count; ++i) {
                snprintf(
                    progress_state.hosts[i].ip,
                    sizeof(progress_state.hosts[i].ip),
                    "%s",
                    request->targets->items[i].ip
                );
            ...
            if (pthread_create(
                ...
                cmaper_log(
                    request->logger,
                    CMAPER_LOG_INFO,
                    "scan/detail-progress: live table enabled (interval=%llds, heartbeat logs suppressed)",
