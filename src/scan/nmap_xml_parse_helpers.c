...
int cmaper_nmap_xml_attr_int(const xmlNode *node, const char *name,
                             int default_value) {
  xmlChar *value;
  char *end = NULL;
  long parsed;
...
void cmaper_nmap_xml_diag_setf(cmaper_nmap_xml_diag_t *diag, const char *field,
                               const char *fmt, ...) {
  va_list args;
...
cmaper_err_t cmaper_nmap_xml_append_script(cmaper_nmap_xml_script_t **items,
                                           size_t *count,
                                           cmaper_nmap_xml_script_t script) {
  cmaper_nmap_xml_script_t *next;

  next = (cmaper_nmap_xml_script_t *)realloc(
      *items, (*count + 1U) * sizeof(cmaper_nmap_xml_script_t));
  if (next == NULL) {
...
cmaper_err_t cmaper_nmap_xml_append_address(cmaper_nmap_xml_address_t **items,
                                            size_t *count,
                                            cmaper_nmap_xml_address_t address) {
  cmaper_nmap_xml_address_t *next;

  next = (cmaper_nmap_xml_address_t *)realloc(
      *items, (*count + 1U) * sizeof(cmaper_nmap_xml_address_t));
  if (next == NULL) {
    return CMAPER_ERR_OOM;
  }
...
cmaper_err_t
cmaper_nmap_xml_append_hostname(cmaper_nmap_xml_hostname_t **items,
                                size_t *count,
                                cmaper_nmap_xml_hostname_t hostname) {
  cmaper_nmap_xml_hostname_t *next;

  next = (cmaper_nmap_xml_hostname_t *)realloc(
      *items, (*count + 1U) * sizeof(cmaper_nmap_xml_hostname_t));
  if (next == NULL) {
    return CMAPER_ERR_OOM;
  }
...
cmaper_err_t cmaper_nmap_xml_append_port(cmaper_nmap_xml_port_t **items,
                                         size_t *count,
                                         cmaper_nmap_xml_port_t port) {
  cmaper_nmap_xml_port_t *next;

  next = (cmaper_nmap_xml_port_t *)realloc(
      *items, (*count + 1U) * sizeof(cmaper_nmap_xml_port_t));
  if (next == NULL) {
    return CMAPER_ERR_OOM;
  }
...
cmaper_err_t cmaper_nmap_xml_append_osmatch(cmaper_nmap_xml_osmatch_t **items,
                                            size_t *count,
                                            cmaper_nmap_xml_osmatch_t osmatch) {
  cmaper_nmap_xml_osmatch_t *next;

  next = (cmaper_nmap_xml_osmatch_t *)realloc(
      *items, (*count + 1U) * sizeof(cmaper_nmap_xml_osmatch_t));
  if (next == NULL) {
    return CMAPER_ERR_OOM;
  }
...
cmaper_err_t
cmaper_nmap_xml_append_trace_hop(cmaper_nmap_xml_trace_hop_t **items,
                                 size_t *count,
                                 cmaper_nmap_xml_trace_hop_t hop) {
  cmaper_nmap_xml_trace_hop_t *next;

  next = (cmaper_nmap_xml_trace_hop_t *)realloc(
      *items, (*count + 1U) * sizeof(cmaper_nmap_xml_trace_hop_t));
  if (next == NULL) {
    return CMAPER_ERR_OOM;
  }
...
cmaper_err_t cmaper_nmap_xml_append_host(cmaper_nmap_xml_document_t *document,
                                         cmaper_nmap_xml_host_t host) {
  cmaper_nmap_xml_host_t *next;

  next = (cmaper_nmap_xml_host_t *)realloc(document->hosts,
                                           (document->host_count + 1U) *
                                               sizeof(cmaper_nmap_xml_host_t));
  if (next == NULL) {
    return CMAPER_ERR_OOM;
  }
...
cmaper_err_t
cmaper_nmap_xml_parse_script_node(const xmlNode *script_node,
                                  cmaper_nmap_xml_script_t *script) {
  if (script == NULL) {
