...
cmaper_err_t cmaper_preflight_run(
    ...
    report->nmap_found = paths->nmap_bin[0] != '\0' && cmaper_fs_path_is_executable(paths->nmap_bin);
    if (report->nmap_found) {
        cmaper_log(logger, CMAPER_LOG_OK, "preflight: nmap found at '%s'", paths->nmap_bin);
    } else {
        cmaper_log(logger, CMAPER_LOG_FAIL, "preflight: nmap binary was not resolved");
    }

    cmaper_log(logger, CMAPER_LOG_WAIT, "preflight: ensuring runtime directories");
    rc = cmaper_fs_ensure_directory_recursive(paths->db_dir);
    report->db_dir_ready = (rc == CMAPER_OK) && cmaper_fs_path_is_directory(paths->db_dir);

    rc = cmaper_fs_ensure_directory_recursive(paths->xml_output_dir);
    report->xml_output_dir_ready = (rc == CMAPER_OK) && cmaper_fs_path_is_directory(paths->xml_output_dir);

    if (paths->has_nmap_scripts_dir) {
        report->scripts_dir_ready = cmaper_fs_path_is_directory(paths->nmap_scripts_dir);
    } else {
        report->scripts_dir_ready = false;
    }

    cmaper_log(logger, report->db_dir_ready ? CMAPER_LOG_OK : CMAPER_LOG_FAIL,
        "preflight: db-dir '%s' => %s",
        paths->db_dir,
        cmaper_preflight_ok(report->db_dir_ready));
    cmaper_log(logger, report->xml_output_dir_ready ? CMAPER_LOG_OK : CMAPER_LOG_FAIL,
        "preflight: xml-dir '%s' => %s",
        paths->xml_output_dir,
        cmaper_preflight_ok(report->xml_output_dir_ready));

    if (paths->has_nmap_scripts_dir) {
        cmaper_log(logger, report->scripts_dir_ready ? CMAPER_LOG_OK : CMAPER_LOG_WARN,
            "preflight: scripts-dir '%s' => %s",
            paths->nmap_scripts_dir,
            report->scripts_dir_ready ? "ok" : "missing");
    } else {
        cmaper_log(logger, CMAPER_LOG_WARN, "preflight: scripts-dir was not resolved");
    }

    cmaper_log(logger, CMAPER_LOG_WAIT, "preflight: checking helper/runtime sanity");
    report->helper_selftest_ok =
        paths->db_path[0] != '\0'
        && paths->db_dir[0] != '\0'
        && paths->xml_output_dir[0] != '\0';
    cmaper_log(logger, report->helper_selftest_ok ? CMAPER_LOG_OK : CMAPER_LOG_FAIL,
        "preflight: helper-runtime-sanity => %s",
        cmaper_preflight_ok(report->helper_selftest_ok));

    cmaper_log(logger, CMAPER_LOG_WAIT, "preflight: checking stdout/stderr separation");
    stdout_fd = fileno(stdout);
    stderr_fd = fileno(stderr);
    report->stdout_available = stdout_fd >= 0;
    report->stderr_available = stderr_fd >= 0;
    report->stdout_stderr_separated =
        report->stdout_available && report->stderr_available && stdout_fd != stderr_fd;

    cmaper_log(logger, report->stdout_stderr_separated ? CMAPER_LOG_OK : CMAPER_LOG_FAIL,
...
void cmaper_preflight_render_report(
    FILE *stream,
    ...
    fprintf(stream,
        ...
        cmaper_preflight_status_word(report->success),
        nmap_bin,
        cmaper_preflight_status_word(report->nmap_found),
        scripts_dir,
        cmaper_preflight_status_word(report->scripts_dir_ready),
        paths->db_path,
        cmaper_preflight_status_word(report->db_dir_ready),
        paths->xml_output_dir,
