...
static cmaper_err_t cmaper_platform_clipboard_run_command(
    const char *command,
    const char *data,
    size_t data_size
) {
...
cmaper_err_t cmaper_platform_clipboard_copy(
    const char *data,
    size_t data_size
) {
    ...
    for (i = 0; i < sizeof(COMMANDS) / sizeof(COMMANDS[0]); ++i) {
        cmaper_err_t rc = cmaper_platform_clipboard_run_command(
            COMMANDS[i],
            data,
            data_size
        );
