...
bool cmaper_security_contains_ci(const char *haystack, const char *needle) {
  ...
  for (i = 0; haystack[i] != '\0'; ++i) {
    ...
    for (j = 0; j < needle_len; ++j) {
      if (haystack[i + j] == '\0' ||
          !cmaper_security_char_eq_ci(haystack[i + j], needle[j])) {
        match = false;
        break;
      }
    }
...
size_t cmaper_security_normalize_spaces(const char *input, char *out,
  ...
  for (; input[i] != '\0'; ++i) {
    ...
    out[out_len++] =
        (char)(lower_case ? cmaper_security_ascii_tolower(ch) : ch);
  }
...
bool cmaper_security_extract_labeled_value(const char *text, const char *label,
  ...
  for (i = 0; text[i] != '\0'; ++i) {
    ...
    for (j = 0; j < label_len; ++j) {
      if (text[i + j] == '\0' ||
          !cmaper_security_char_eq_ci(text[i + j], label[j])) {
        matches = false;
        break;
      }
    }
    ...
    while (text[start] != '\0' &&
           !cmaper_security_token_is_boundary(text[start])) {
      if (out_len + 1U >= out_cap) {
        break;
      }
      out[out_len++] =
          (char)cmaper_security_ascii_tolower((unsigned char)text[start]);
      start += 1U;
    }
    out[out_len] = '\0';
    if (out_len > 0) {
...
bool cmaper_security_extract_hex_colon_token(const char *text, char *out,
  ...
  for (i = 0; text[i] != '\0'; ++i) {
    ...
    if (len >= 17U && colon_count >= 5U && len + 1U < out_cap) {
      ...
      for (k = 0; k < len; ++k) {
        out[k] =
            (char)cmaper_security_ascii_tolower((unsigned char)text[i + k]);
      }
      out[len] = '\0';
      return true;
    }
...
bool cmaper_security_extract_line_value_ci(const char *text, const char *label,
  ...
  for (i = 0; text[i] != '\0'; ++i) {
    ...
    while (text[value_start] != '\0' && text[value_start] != '\n' &&
           text[value_start] != '\r') {
      if (out_len + 1U >= out_cap) {
        break;
      }
