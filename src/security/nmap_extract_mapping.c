...
static void cmaper_security_copy_protocol(char *out, size_t out_cap,
                                          const char *protocol) {
...
static bool cmaper_security_context_matches(
    bool left_has_context, const char *left_protocol, int left_port,
    bool right_has_context, const char *right_protocol, int right_port) {
...
cmaper_err_t cmaper_security_nmap_append_fingerprint(
    ...
    const cmaper_security_fingerprint_t *fingerprint, bool has_service_context,
    const char *protocol, int port) {
  cmaper_security_fingerprint_observation_t *next;
  size_t i;

  if (artifacts == NULL || fingerprint == NULL ||
      fingerprint->value[0] == '\0') {
    return CMAPER_ERR_INVALID_ARGUMENT;
  }

  for (i = 0; i < artifacts->fingerprint_count; ++i) {
    const cmaper_security_fingerprint_observation_t *existing =
    ...
    if (!cmaper_security_context_matches(existing->has_service_context,
                                         existing->protocol, existing->port,
                                         has_service_context, protocol, port)) {
      continue;
    }
    return CMAPER_OK;
  }
  ...
  next = (cmaper_security_fingerprint_observation_t *)realloc(
      ...
      (artifacts->fingerprint_count + 1U) *
          sizeof(cmaper_security_fingerprint_observation_t));
  if (next == NULL) {
    return CMAPER_ERR_OOM;
  }
  artifacts->fingerprints = next;

  artifacts->fingerprints[artifacts->fingerprint_count].fingerprint =
      *fingerprint;
  artifacts->fingerprints[artifacts->fingerprint_count].has_service_context =
      has_service_context;
  artifacts->fingerprints[artifacts->fingerprint_count].port =
      has_service_context ? port : 0;
...
cmaper_err_t
cmaper_security_nmap_append_finding(cmaper_security_host_artifacts_t *artifacts,
  ...
  for (i = 0; i < artifacts->finding_count; ++i) {
    const cmaper_security_finding_observation_t *existing =
  ...
  next = (cmaper_security_finding_observation_t *)realloc(
      artifacts->findings, (artifacts->finding_count + 1U) *
                               sizeof(cmaper_security_finding_observation_t));
  if (next == NULL) {
    return CMAPER_ERR_OOM;
  }
  ...
  artifacts->findings[artifacts->finding_count].has_service_context =
      has_service_context;
  artifacts->findings[artifacts->finding_count].port =
      has_service_context ? port : 0;
  cmaper_security_copy_protocol(
...
cmaper_err_t cmaper_security_nmap_append_surface(
    ...
    bool has_service_context, const char *protocol, int port) {
  ...
  for (i = 0; i < artifacts->surface_count; ++i) {
    const cmaper_security_surface_observation_t *existing =
  ...
  next = (cmaper_security_surface_observation_t *)realloc(
      artifacts->surfaces, (artifacts->surface_count + 1U) *
                               sizeof(cmaper_security_surface_observation_t));
  if (next == NULL) {
    return CMAPER_ERR_OOM;
  }
  ...
  artifacts->surfaces[artifacts->surface_count].has_service_context =
      has_service_context;
  artifacts->surfaces[artifacts->surface_count].port =
      has_service_context ? port : 0;
  cmaper_security_copy_protocol(
