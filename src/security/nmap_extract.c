...
void cmaper_security_host_artifacts_init(
...
void cmaper_security_host_artifacts_dispose(
    cmaper_security_host_artifacts_t *artifacts) {
  if (artifacts == NULL) {
    return;
  }
...
cmaper_err_t cmaper_security_extract_from_host_pair(
  ...
  for (i = 0; i < port_count; ++i) {
    ...
    if (port->portid <= 0 || port->protocol == NULL ||
        port->protocol[0] == '\0') {
      continue;
    }

    surface_count = cmaper_security_detect_management_surfaces_for_port(
        port->protocol, port->portid, port->service_name, surfaces,
        sizeof(surfaces) / sizeof(surfaces[0]));
    for (j = 0; j < surface_count; ++j) {
      rc = cmaper_security_nmap_append_surface(
          out_artifacts, &surfaces[j], true, port->protocol, port->portid);
      if (rc != CMAPER_OK) {
        cmaper_security_host_artifacts_dispose(out_artifacts);
        return rc;
      }
    ...
    for (j = 0; j < port->script_count; ++j) {
      ...
      rc =
          cmaper_security_nmap_extract_from_script(out_artifacts, &input, true);
      if (rc != CMAPER_OK) {
        cmaper_security_host_artifacts_dispose(out_artifacts);
        return rc;
      }
