...
static bool cmaper_cli_print_mode_help(FILE *stream, const char *program_name,
                                       const char *topic) {
  if (topic == NULL) {
    return false;
  }
  ...
  if (strcmp(topic, "scan") == 0) {
    fprintf(
        stream,
        "Usage: %s scan --target <target> [--profile <low|mid|high>] "
        "[scan-options]\n"
  ...
  if (strcmp(topic, "timeline") == 0) {
    fprintf(stream,
            "Usage: %s timeline --session <session-id> [--device <device-id>] "
            "[--limit <n>]\n"
  ...
  if (strcmp(topic, "device") == 0) {
    fprintf(stream,
            ...
            "[--window <days|daysd>] [--changes-only]\n"
            "       %s device <session-id> <device-id> [--window <days|daysd>] "
            "[--changes-only]\n",
            program_name, program_name);
    return true;
  }
...
void cmaper_cli_print_help(FILE *stream, const char *program_name,
  ...
  fprintf(
      ...
      "\n"
      "Modes:\n"
      "  scan                 Run discovery + detail scan and persist session "
      "history\n"
      ...
      "  timeline             Timeline around a session (optional device "
      "filter)\n"
      ...
      "  delete-session       Delete a single session (interactive "
      "confirmation)\n"
      ...
      "      --dev              Run preflight before executing the selected "
      "mode\n"
      "      --xml-only         Scan without SQLite persistence (XML artifacts "
      "only)\n"
      "  -v, --verbose          Increase log verbosity (shows phase/info "
      "diagnostics)\n"
      ...
      "      --spoof-mac <value>    Enable spoofing with specific MAC/random "
      "token\n"
      ...
      "      --window <days|daysd>  Device mode: include only the last N days "
      "of history before the selected session\n"
      "      --changes-only         Device mode: show only context and change "
      "timeline sections\n"
      "      --yes                  Compatibility flag for destructive mode "
      "automation\n"
      "\n"
      "Examples:\n"
      ...
      "  %s devices <session-id> --format json --output "
      "file:/tmp/devices.json\n",
