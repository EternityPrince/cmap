...
static const char *cmaper_cli_inline_value(const char *arg, const char *option) {
    size_t option_len;
...
static cmaper_err_t cmaper_cli_take_option_value(
    int argc,
    char **argv,
    int *index,
    const char *option,
...
static cmaper_err_t cmaper_cli_assign_once(
    const char **slot,
    const char *value,
    const char *option,
    cmaper_cli_diagnostic_t *diag
    ...
    if (*slot != NULL) {
        cmaper_cli_diag_setf(diag, option, "option '%s' can be specified only once", option);
        return CMAPER_ERR_CLI_USAGE;
    }
    ...
    for (i = 1; i < argc; ++i) {
        ...
        if (parse_options && arg[0] == '-' && arg[1] != '\0') {
            ...
            if (strcmp(arg, "--target") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--target", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
            ...
            if (strcmp(arg, "--profile") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--profile", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
                rc = cmaper_cli_assign_once(&raw->scan_profile, value, "--profile", diag);
                if (rc != CMAPER_OK) {
                    return rc;
                }
                continue;
            ...
            if (strcmp(arg, "--ports") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--ports", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
            ...
            if (strcmp(arg, "--exact-ports") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--exact-ports", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
                rc = cmaper_cli_assign_once(&raw->scan_ports, value, "--exact-ports", diag);
                if (rc != CMAPER_OK) {
                    return rc;
                }
                continue;
            ...
            if (strcmp(arg, "--timing") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--timing", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
            ...
            if (strcmp(arg, "--detail-workers") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--detail-workers", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
                rc = cmaper_cli_assign_once(
            ...
            if (strcmp(arg, "--spoof-mac") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--spoof-mac", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
                rc = cmaper_cli_assign_once(&raw->spoof_mac, value, "--spoof-mac", diag);
                if (rc != CMAPER_OK) {
                    return rc;
                }
                continue;
            ...
            if (strcmp(arg, "--session") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--session", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
            ...
            if (strcmp(arg, "--session-id") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--session-id", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
                rc = cmaper_cli_assign_once(&raw->session_id, value, "--session-id", diag);
                if (rc != CMAPER_OK) {
                    return rc;
                }
                continue;
            ...
            if (strcmp(arg, "--from") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--from", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
                rc = cmaper_cli_assign_once(&raw->from_session_id, value, "--from", diag);
                if (rc != CMAPER_OK) {
                    return rc;
                }
                continue;
            ...
            if (strcmp(arg, "--to") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--to", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
            ...
            if (strcmp(arg, "--device") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--device", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
            ...
            if (strcmp(arg, "--device-id") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--device-id", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
                rc = cmaper_cli_assign_once(&raw->device_id, value, "--device-id", diag);
                if (rc != CMAPER_OK) {
                    return rc;
                }
                continue;
            ...
            if (strcmp(arg, "--format") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--format", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
            ...
            if (strcmp(arg, "--output") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--output", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
            ...
            if (strcmp(arg, "--view") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--view", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
            ...
            if (strcmp(arg, "--limit") == 0 || inline_value != NULL) {
                rc = cmaper_cli_take_option_value(
                    argc, argv, &i, "--limit", inline_value, &value, diag
                );
                if (rc != CMAPER_OK) {
                    return rc;
                }
