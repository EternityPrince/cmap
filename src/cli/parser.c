...
static cmaper_log_level_t cmaper_cli_log_level_from_delta(int delta) {
   ...
   * One `-v` should immediately enable [PHASE], while one `-q` should hide
   * [WAIT].
   */
  if (delta > 0) {
...
static cmaper_err_t cmaper_cli_parse_int_value(const char *raw_value,
                                               const char *option,
                                               int *out_value,
  ...
  if (raw_value[0] == '\0' || end == NULL || *end != '\0' || errno == ERANGE) {
    cmaper_cli_diag_setf(diag, option, "invalid integer value for '%s': '%s'",
                         option, raw_value);
    return CMAPER_ERR_CLI_USAGE;
  }

  if (parsed < INT_MIN || parsed > INT_MAX) {
    cmaper_cli_diag_setf(diag, option,
...
static cmaper_err_t cmaper_cli_resolve_scan_toggle(
    bool enable_flag, bool disable_flag, const char *enable_option,
    const char *disable_option, cmaper_scan_toggle_t *out_toggle,
    cmaper_cli_diagnostic_t *diag) {
  if (enable_flag && disable_flag) {
    cmaper_cli_diag_setf(diag, enable_option,
...
static cmaper_err_t
cmaper_cli_apply_scan_options(cmaper_cli_config_t *config,
  ...
  rc = cmaper_cli_resolve_scan_toggle(
      raw->enable_all_ports, raw->disable_all_ports, "--all-ports",
      "--no-all-ports", &config->scan.all_ports, diag);
  if (rc != CMAPER_OK) {
    return rc;
  }

  rc = cmaper_cli_resolve_scan_toggle(raw->enable_no_ping, raw->disable_no_ping,
                                      "--no-ping", "--ping",
                                      &config->scan.no_ping, diag);
  if (rc != CMAPER_OK) {
    return rc;
  }

  if (raw->scan_timing != NULL) {
    rc = cmaper_cli_parse_int_value(raw->scan_timing, "--timing",
                                    &config->scan.timing_template, diag);
    if (rc != CMAPER_OK) {
      return rc;
    }
  ...
  if (raw->scan_detail_workers != NULL) {
    rc =
        cmaper_cli_parse_int_value(raw->scan_detail_workers, "--detail-workers",
                                   &config->scan.detail_workers, diag);
    if (rc != CMAPER_OK) {
      return rc;
    }
  ...
  rc = cmaper_cli_resolve_scan_toggle(
      raw->enable_service_detection, raw->disable_service_detection,
      "--service-detection", "--no-service-detection",
      &config->scan.service_detection, diag);
  if (rc != CMAPER_OK) {
    return rc;
  }

  rc = cmaper_cli_resolve_scan_toggle(
      raw->enable_os_detection, raw->disable_os_detection, "--os-detection",
      "--no-os-detection", &config->scan.os_detection, diag);
  if (rc != CMAPER_OK) {
    return rc;
  }

  rc = cmaper_cli_resolve_scan_toggle(raw->enable_sudo, raw->disable_sudo,
                                      "--sudo", "--no-sudo", &config->scan.sudo,
                                      diag);
  if (rc != CMAPER_OK) {
    return rc;
  }
  ...
  rc = cmaper_cli_resolve_scan_toggle(
      raw->enable_traceroute, raw->disable_traceroute, "--traceroute",
      "--no-traceroute", &config->scan.traceroute, diag);
  if (rc != CMAPER_OK) {
    return rc;
  }

  rc = cmaper_cli_resolve_scan_toggle(raw->enable_udp_enrichment,
                                      raw->disable_udp_enrichment,
                                      "--udp-enrichment", "--no-udp-enrichment",
                                      &config->scan.udp_enrichment, diag);
  if (rc != CMAPER_OK) {
    return rc;
  }
...
static cmaper_err_t
cmaper_cli_apply_output_options(cmaper_cli_config_t *config,
                                const cmaper_cli_raw_args_t *raw,
                                cmaper_cli_diagnostic_t *diag) {
  cmaper_output_format_t parsed_format = CMAPER_OUTPUT_FORMAT_TERMINAL;

  if (raw->format != NULL) {
    if (strcmp(raw->format, "terminal") == 0 ||
        strcmp(raw->format, "text") == 0) {
      parsed_format = CMAPER_OUTPUT_FORMAT_TERMINAL;
    } else if (strcmp(raw->format, "markdown") == 0 ||
               strcmp(raw->format, "md") == 0) {
      parsed_format = CMAPER_OUTPUT_FORMAT_MARKDOWN;
    } else if (strcmp(raw->format, "json") == 0) {
      parsed_format = CMAPER_OUTPUT_FORMAT_JSON;
    } else {
      cmaper_cli_diag_setf(diag, "--format",
                           "unsupported value for '--format': '%s' (expected: "
  ...
  if (raw->wants_color && raw->wants_no_color) {
    cmaper_cli_diag_set(
        diag, "--color",
        "options '--color' and '--no-color' cannot be used together");
    return CMAPER_ERR_CLI_USAGE;
  }

  config->output.log_level =
      cmaper_cli_log_level_from_delta(raw->verbosity_delta);
  ...
  if (raw->use_json_shortcut && raw->format != NULL &&
      parsed_format != CMAPER_OUTPUT_FORMAT_JSON) {
    cmaper_cli_diag_set(
        diag, "--format",
  ...
  if (raw->output != NULL) {
    if (strcmp(raw->output, "terminal") == 0) {
      ...
      cmaper_cli_diag_setf(diag, "--output",
                           "unsupported value for '--output': '%s' (expected: "
                           "terminal|clipboard|file:<path>)",
                           raw->output);
      return CMAPER_ERR_CLI_USAGE;
    }
  }
...
static cmaper_err_t cmaper_cli_parse_limit(cmaper_cli_config_t *config,
                                           const cmaper_cli_raw_args_t *raw,
                                           cmaper_cli_diagnostic_t *diag) {
  char *end = NULL;
  long parsed = 0;
  ...
  if (raw->limit[0] == '\0' || end == NULL || *end != '\0' || errno == ERANGE) {
    cmaper_cli_diag_setf(diag, "--limit",
...
static cmaper_err_t cmaper_cli_parse_window(cmaper_cli_config_t *config,
                                            const cmaper_cli_raw_args_t *raw,
                                            cmaper_cli_diagnostic_t *diag) {
  char *end = NULL;
  long parsed = 0;
  ...
  errno = 0;
  parsed = strtol(raw->window, &end, 10);
  if (raw->window[0] == '\0' || end == NULL || errno == ERANGE) {
    cmaper_cli_diag_setf(diag, "--window",
                         "invalid value for '--window': '%s' (expected: <days> or <days>d)",
                         raw->window);
    return CMAPER_ERR_CLI_USAGE;
  }

  if (*end != '\0' && !(end[0] == 'd' && end[1] == '\0') &&
      !(end[0] == 'D' && end[1] == '\0')) {
    cmaper_cli_diag_setf(diag, "--window",
                         "invalid value for '--window': '%s' (expected: <days> or <days>d)",
                         raw->window);
  ...
  if (parsed <= 0 || parsed > INT_MAX) {
    cmaper_cli_diag_setf(diag, "--window",
                         "option '--window' must be a positive day count");
    return CMAPER_ERR_CLI_USAGE;
  }

  config->history.window_days = (int)parsed;
  config->history.has_window_days = true;
  return CMAPER_OK;
}

static cmaper_err_t
cmaper_cli_unexpected_positionals(const cmaper_cli_raw_args_t *raw,
                                  size_t consumed,
                                  cmaper_cli_diagnostic_t *diag) {
  ...
  cmaper_cli_diag_setf(diag, raw->positionals[consumed],
...
static cmaper_err_t
cmaper_cli_apply_mode_positionals(cmaper_cli_config_t *config,
                                  const cmaper_cli_raw_args_t *raw,
                                  cmaper_cli_diagnostic_t *diag) {
  size_t idx = 0;
  ...
  case CMAPER_CLI_MODE_DIFF_GLOBAL:
    if (config->history.from_session_id == NULL &&
  ...
  if ((config->mode == CMAPER_CLI_MODE_TIMELINE ||
       config->mode == CMAPER_CLI_MODE_POSTURE) &&
      config->history.device_id == NULL && idx < raw->positional_count) {
    config->history.device_id = raw->positionals[idx++];
  }
...
static cmaper_err_t cmaper_cli_apply_intent(cmaper_cli_config_t *config,
                                            const cmaper_cli_raw_args_t *raw,
                                            cmaper_cli_diagnostic_t *diag) {
  ...
  if (raw->wants_version) {
    if (raw->mode_token != NULL || raw->positional_count > 0) {
      cmaper_cli_diag_set(
          diag, "--version",
          "option '--version' does not accept command arguments");
      return CMAPER_ERR_CLI_USAGE;
    }
  ...
  if (raw->wants_help) {
    ...
    if (raw->positional_count > 0) {
      cmaper_cli_diag_set(diag, raw->positionals[0],
  ...
  if (raw->wants_check) {
    if (raw->mode_token != NULL || raw->positional_count > 0) {
      cmaper_cli_diag_set(diag, "--check",
  ...
  if (strcmp(raw->mode_token, "help") == 0) {
    ...
    if (raw->positional_count > 1) {
      cmaper_cli_diag_set(diag, raw->positionals[1],
                          "too many arguments for help");
      return CMAPER_ERR_CLI_USAGE;
    }
    config->help_topic =
        raw->positional_count == 1 ? raw->positionals[0] : NULL;
    return CMAPER_OK;
  }
  ...
  if (strcmp(raw->mode_token, "version") == 0) {
    ...
    if (raw->positional_count > 0) {
      cmaper_cli_diag_set(
          diag, raw->positionals[0],
          "command 'version' does not accept positional arguments");
      return CMAPER_ERR_CLI_USAGE;
    }
    return CMAPER_OK;
  }
  ...
  if (!cmaper_cli_mode_is_defined(config->mode)) {
    cmaper_cli_diag_setf(diag, raw->mode_token, "unknown mode '%s'",
                         raw->mode_token);
    return CMAPER_ERR_CLI_USAGE;
  }
...
cmaper_err_t cmaper_cli_normalize_config(cmaper_cli_config_t *config,
  ...
  config->history.changes_only = raw->changes_only;
  ...
  rc = cmaper_cli_parse_window(config, raw, diag);
  if (rc != CMAPER_OK) {
    return rc;
  }
