...
static void cmaper_runtime_paths_diag_setf(
...
static cmaper_err_t cmaper_runtime_copy_string(
    ...
    if (written < 0 || (size_t) written >= out_cap) {
        cmaper_runtime_paths_diag_setf(
            diag,
            field,
            "resolved path for '%s' exceeds internal buffer limit",
            field
        );
...
static cmaper_err_t cmaper_runtime_join_path(
    char *out,
    size_t out_cap,
    const char *left,
    const char *right,
...
static cmaper_err_t cmaper_runtime_find_in_path(
    ...
    if (binary_name == NULL || binary_name[0] == '\0' || out_path == NULL || out_cap == 0) {
        return CMAPER_ERR_INVALID_ARGUMENT;
    }
    ...
    while (cursor[0] != '\0') {
        ...
        if (dir_len > 0 && dir_len < sizeof(dir)) {
            ...
            written = snprintf(candidate, sizeof(candidate), "%s/%s", dir, binary_name);
            if (written > 0
                && (size_t) written < sizeof(candidate)
                && cmaper_fs_path_is_executable(candidate)) {
                return cmaper_runtime_copy_string(out_path, out_cap, candidate, NULL, NULL);
            }
        }
...
static void cmaper_runtime_try_default_scripts_dir(cmaper_runtime_paths_t *paths) {
    ...
    for (i = 0; i < (sizeof(candidates) / sizeof(candidates[0])); ++i) {
        if (cmaper_fs_path_is_directory(candidates[i])) {
            snprintf(paths->nmap_scripts_dir, sizeof(paths->nmap_scripts_dir), "%s", candidates[i]);
...
const char *cmaper_runtime_env_nmap_scripts_dir(void) {
    return cmaper_runtime_getenv_compat("NMAPER_NMAP_SCRIPTS_DIR", "CMAPER_NMAP_SCRIPTS_DIR");
}
...
cmaper_err_t cmaper_runtime_paths_resolve(
    ...
    xml_env = cmaper_runtime_getenv_compat("NMAPER_XML_OUTPUT_DIR", "CMAPER_XML_OUTPUT_DIR");
    nmap_env = cmaper_runtime_env_nmap_bin();
    scripts_env = cmaper_runtime_env_nmap_scripts_dir();
    data_root_env = cmaper_runtime_getenv_compat("NMAPER_DATA_DIR", "CMAPER_DATA_DIR");
    home_dir = cmaper_runtime_getenv_nonempty("HOME");

    if (data_root_env != NULL) {
        if (snprintf(data_root, sizeof(data_root), "%s", data_root_env) >= (int) sizeof(data_root)) {
            cmaper_runtime_paths_diag_setf(diag, "data-root", "resolved data root path is too long");
            return CMAPER_ERR_IO;
        }
    } else if (home_dir != NULL) {
        if (snprintf(data_root, sizeof(data_root), "%s/.local/share/cmaper", home_dir)
            >= (int) sizeof(data_root)) {
            cmaper_runtime_paths_diag_setf(diag, "data-root", "resolved data root path is too long");
            return CMAPER_ERR_IO;
        }
    } else if (getcwd(cwd, sizeof(cwd)) != NULL) {
        if (snprintf(data_root, sizeof(data_root), "%s/.cmaper", cwd) >= (int) sizeof(data_root)) {
            cmaper_runtime_paths_diag_setf(diag, "data-root", "resolved data root path is too long");
            return CMAPER_ERR_IO;
        }
    } else {
        if (snprintf(data_root, sizeof(data_root), ".cmaper") >= (int) sizeof(data_root)) {
            cmaper_runtime_paths_diag_setf(diag, "data-root", "resolved data root path is too long");
            return CMAPER_ERR_IO;
        }
    }
    ...
    if (db_env != NULL) {
        rc = cmaper_runtime_copy_string(paths->db_path, sizeof(paths->db_path), db_env, diag, "db-path");
    } else {
        rc = cmaper_runtime_join_path(
            paths->db_path,
            sizeof(paths->db_path),
            data_root,
            "cmaper.db",
            diag,
            "db-path"
    ...
    if (xml_env != NULL) {
        rc = cmaper_runtime_copy_string(
            paths->xml_output_dir,
            sizeof(paths->xml_output_dir),
            xml_env,
            diag,
            "xml-output-dir"
        );
    ...
    rc = cmaper_fs_parent_directory(paths->db_path, paths->db_dir, sizeof(paths->db_dir));
    if (rc != CMAPER_OK) {
        cmaper_runtime_paths_diag_setf(diag, "db-path", "cannot resolve parent directory for db path");
        return rc;
    }

    if (nmap_env != NULL) {
        paths->nmap_bin_from_env = true;
        rc = cmaper_runtime_copy_string(paths->nmap_bin, sizeof(paths->nmap_bin), nmap_env, diag, "nmap-bin");
    } else {
        rc = cmaper_runtime_find_in_path("nmap", paths->nmap_bin, sizeof(paths->nmap_bin));
    }
    ...
    if (scripts_env != NULL) {
        ...
        if (root_len >= sizeof(inferred_scripts)) {
            cmaper_runtime_paths_diag_setf(diag, "nmap-scripts-dir", "derived scripts path is too long");
            return CMAPER_ERR_IO;
        }
        ...
        paths->has_nmap_scripts_dir = cmaper_fs_path_is_directory(paths->nmap_scripts_dir);
