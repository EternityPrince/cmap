...
static int cmaper_app_usage_error(
...
static int cmaper_app_scan_plan_error(
    const char *program_name,
...
static int cmaper_app_runtime_error(
    const char *program_name,
...
static int cmaper_app_snapshot_error(
    const char *program_name,
...
static void cmaper_app_make_session_uid(char *out, size_t out_cap) {
    time_t now;
    long pid;
...
int app_run(int argc, char **argv) {
    ...
    if (config.mode == CMAPER_CLI_MODE_CHECK || config.dev_mode) {
        rc = cmaper_preflight_run(&runtime.paths, &runtime.logger, &preflight_report);
    ...
    if (config.mode == CMAPER_CLI_MODE_SCAN) {
        ...
        rc = cmaper_snapshot_session_start(&snapshot_store, &snapshot_session_start);
        if (rc != CMAPER_OK) {
        ...
        if (rc != CMAPER_OK) {
            ...
            (void) cmaper_snapshot_session_fail(&snapshot_store, &snapshot_session_fail);
        ...
        rc = cmaper_snapshot_store_write_scan(&snapshot_store, &snapshot_write_request, &runtime.logger);
        if (rc != CMAPER_OK) {
            snapshot_session_fail.session_uid = session_uid;
        ...
        snapshot_session_complete.discovery_xml_path = scan_result.discovery_xml_path;
        snapshot_session_complete.detail_targets_total = scan_result.detail_targets.count;
        snapshot_session_complete.detail_hosts_success = scan_result.detail_result.successful_hosts;
        snapshot_session_complete.detail_hosts_failed = scan_result.detail_result.failed_hosts;
        snapshot_session_complete.detail_hosts_degraded = scan_result.detail_result.degraded_hosts;
        rc = cmaper_snapshot_session_complete(&snapshot_store, &snapshot_session_complete);
        if (rc != CMAPER_OK) {
            snapshot_session_fail.session_uid = session_uid;
