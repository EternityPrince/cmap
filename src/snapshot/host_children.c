...
static cmaper_err_t cmaper_snapshot_upsert_service_observation(
    ...
    *out_service_observation_id = 0;
    ...
    rc = cmaper_snapshot_sqlite_bind_text_or_null(stmt_upsert, 5, port->service_name);
    if (rc != CMAPER_OK) {
        goto cleanup;
    }
    rc = cmaper_snapshot_sqlite_bind_text_or_null(stmt_upsert, 6, port->service_product);
    if (rc != CMAPER_OK) {
        goto cleanup;
    }
    rc = cmaper_snapshot_sqlite_bind_text_or_null(stmt_upsert, 7, port->service_version);
    if (rc != CMAPER_OK) {
        goto cleanup;
    }
...
cmaper_err_t cmaper_snapshot_replace_host_children(
    ...
    for (i = 0; i < host_view->os_count; ++i) {
        rc = cmaper_snapshot_insert_os_match(db, host_observation_id, &host_view->os_matches[i]);
        if (rc != CMAPER_OK) {
            return rc;
        }
    }
    ...
    for (i = 0; i < host_view->port_count; ++i) {
        ...
        if (port->protocol == NULL || port->protocol[0] == '\0' || port->portid <= 0) {
            continue;
        }
    ...
    if (host_view->trace_count > 0 && host_view->trace_hops != NULL) {
        rc = cmaper_snapshot_insert_trace(
            db,
            host_observation_id,
            host_view->trace_hops,
            host_view->trace_count
        );
        if (rc != CMAPER_OK) {
            return rc;
        }
