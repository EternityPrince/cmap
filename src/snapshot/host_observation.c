...
cmaper_err_t cmaper_snapshot_upsert_host_observation(
    ...
    *out_host_observation_id = 0;
    ...
    rc = cmaper_snapshot_sqlite_bind_text_or_null(stmt_upsert, 4, primary_ip_type);
    if (rc != CMAPER_OK) {
        goto cleanup;
    }
    ...
    rc = cmaper_snapshot_sqlite_bind_text_or_null(stmt_upsert, 7, hostname_primary);
    if (rc != CMAPER_OK) {
        goto cleanup;
    }
    ...
    rc = cmaper_snapshot_sqlite_bind_text_or_null(stmt_upsert, 10, detail_xml_path);
    if (rc != CMAPER_OK) {
        goto cleanup;
    }
