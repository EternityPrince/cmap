...
static cmaper_err_t
cmaper_snapshot_security_bind_optional_int64(sqlite3_stmt *stmt, int index,
                                             sqlite3_int64 value) {
  int rc;
...
static cmaper_err_t cmaper_snapshot_security_lookup_service_observation_id(
    sqlite3_stmt *stmt_lookup, sqlite3_int64 host_observation_id,
    const char *protocol, int port, sqlite3_int64 *out_service_observation_id) {
  int step_rc;
  cmaper_err_t rc;

  if (stmt_lookup == NULL || protocol == NULL ||
      out_service_observation_id == NULL || port <= 0) {
    return CMAPER_ERR_INVALID_ARGUMENT;
  }
...
static cmaper_err_t cmaper_snapshot_security_insert_fingerprint(
    sqlite3_stmt *stmt_insert, sqlite3_int64 host_observation_id,
    sqlite3_int64 service_observation_id, const char *fingerprint) {
  cmaper_err_t rc;
  ...
  rc = cmaper_snapshot_security_bind_optional_int64(stmt_insert, 2,
...
static cmaper_err_t cmaper_snapshot_security_insert_finding(
  ...
  rc = cmaper_snapshot_sqlite_bind_text(
      stmt_insert, 4, cmaper_security_severity_name(finding->severity));
  if (rc != CMAPER_OK) {
    return rc;
  }
  rc = cmaper_snapshot_sqlite_bind_text(
      stmt_insert, 5, cmaper_security_finding_state_name(finding->state));
  if (rc != CMAPER_OK) {
    return rc;
  }
  ...
  rc =
      cmaper_snapshot_sqlite_bind_text_or_null(stmt_insert, 7, finding->detail);
  if (rc != CMAPER_OK) {
    return rc;
  }
...
static cmaper_err_t cmaper_snapshot_security_insert_surface(
  ...
  rc =
      cmaper_snapshot_sqlite_bind_text_or_null(stmt_insert, 4, surface->detail);
  if (rc != CMAPER_OK) {
    return rc;
  }
...
cmaper_err_t cmaper_snapshot_security_persist_host_artifacts(
  ...
  static const char *SQL_TLS =
      "INSERT INTO tls_fingerprints(host_observation_id, "
      "service_observation_id, fingerprint) "
      "VALUES(?, ?, ?);";
  static const char *SQL_SSH =
      "INSERT INTO ssh_fingerprints(host_observation_id, "
      "service_observation_id, fingerprint) "
      "VALUES(?, ?, ?);";
  static const char *SQL_HTTP =
      "INSERT INTO http_fingerprints(host_observation_id, "
      "service_observation_id, fingerprint) "
      "VALUES(?, ?, ?);";
  static const char *SQL_SMB =
      "INSERT INTO smb_fingerprints(host_observation_id, "
      "service_observation_id, fingerprint) "
      "VALUES(?, ?, ?);";
  static const char *SQL_FINDING =
      "INSERT INTO vulnerability_findings("
      "  host_observation_id, service_observation_id, finding_key, severity, "
      "state, title, detail"
      ") VALUES(?, ?, ?, ?, ?, ?, ?);";
  ...
  rc = cmaper_snapshot_sqlite_prepare(db, SQL_LOOKUP_SERVICE,
                                      &stmt_lookup_service);
  if (rc != CMAPER_OK) {
    goto cleanup;
  }
  ...
  for (i = 0; i < artifacts->fingerprint_count; ++i) {
    const cmaper_security_fingerprint_observation_t *observation =
        &artifacts->fingerprints[i];
    sqlite3_int64 service_observation_id = 0;
    sqlite3_stmt *stmt_insert = NULL;

    if (observation->has_service_context && observation->port > 0 &&
        observation->protocol[0] != '\0') {
      rc = cmaper_snapshot_security_lookup_service_observation_id(
          stmt_lookup_service, host_observation_id, observation->protocol,
          observation->port, &service_observation_id);
      if (rc != CMAPER_OK) {
        goto cleanup;
      }
    }
    ...
    rc = cmaper_snapshot_security_insert_fingerprint(
        stmt_insert, host_observation_id, service_observation_id,
  ...
  for (i = 0; i < artifacts->finding_count; ++i) {
    const cmaper_security_finding_observation_t *observation =
        &artifacts->findings[i];
    sqlite3_int64 service_observation_id = 0;
    ...
    rc = cmaper_snapshot_security_insert_finding(
        stmt_finding, host_observation_id, service_observation_id,
        &observation->finding);
    if (rc != CMAPER_OK) {
      goto cleanup;
    }
  ...
  for (i = 0; i < artifacts->surface_count; ++i) {
    const cmaper_security_surface_observation_t *observation =
        &artifacts->surfaces[i];
    sqlite3_int64 service_observation_id = 0;
    ...
    rc = cmaper_snapshot_security_insert_surface(
        stmt_surface, host_observation_id, service_observation_id,
        &observation->surface);
    if (rc != CMAPER_OK) {
      goto cleanup;
    }
  ...
  if (logger != NULL) {
    cmaper_log(logger, CMAPER_LOG_INFO,
               "snapshot/security: host=%lld fingerprints=%zu findings=%zu "
               "surfaces=%zu",
               (long long)host_observation_id, artifacts->fingerprint_count,
               artifacts->finding_count, artifacts->surface_count);
  }
