...
static const char *CMAPER_SNAPSHOT_SCHEMA_V1[] = {
    ...
    "  service_detection INTEGER NOT NULL DEFAULT 0 CHECK (service_detection "
    "IN (0,1)),"
    ...
    "  udp_enrichment INTEGER NOT NULL DEFAULT 0 CHECK (udp_enrichment IN "
    "(0,1)),"
    ...
    "  first_seen_session_id INTEGER REFERENCES scan_sessions(id) ON DELETE "
    "SET NULL,"
    "  last_seen_session_id INTEGER REFERENCES scan_sessions(id) ON DELETE SET "
    "NULL,"
    "  created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now'))"
    ");",

    "CREATE TABLE IF NOT EXISTS session_networks ("
    "  session_id INTEGER NOT NULL REFERENCES scan_sessions(id) ON DELETE "
    "CASCADE,"
    ...
    "  id INTEGER PRIMARY KEY AUTOINCREMENT,"
    "  host_observation_id INTEGER NOT NULL REFERENCES host_observations(id) "
    "ON DELETE CASCADE,"
    "  name TEXT NOT NULL,"
    "  accuracy INTEGER,"
    "  line INTEGER"
    ...
    "  id INTEGER PRIMARY KEY AUTOINCREMENT,"
    "  host_observation_id INTEGER REFERENCES host_observations(id) ON DELETE "
    "CASCADE,"
    "  service_observation_id INTEGER REFERENCES service_observations(id) ON "
    "DELETE CASCADE,"
    "  script_id TEXT NOT NULL,"
    "  output TEXT,"
    "  created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),"
    "  CHECK (host_observation_id IS NOT NULL OR service_observation_id IS NOT "
    "NULL)"
    ");",
...
static const char *CMAPER_SNAPSHOT_SCHEMA_V2[] = {
    ...
    "idx_vulnerability_findings_service_observation_id"
...
static cmaper_err_t cmaper_snapshot_schema_get_user_version(sqlite3 *db,
                                                            int *out_version) {
...
static cmaper_err_t cmaper_snapshot_schema_apply_migration(
    sqlite3 *db, const cmaper_snapshot_migration_t *migration,
  ...
  for (i = 0; i < migration->statement_count; ++i) {
    ...
    if (rc != CMAPER_OK) {
      cmaper_snapshot_schema_diag_setf(
          diag, "schema", "migration v%d statement #%zu failed: %s",
          migration->version, i + 1U, sqlite3_errmsg(db));
      sqlite3_exec(db, "ROLLBACK;", NULL, NULL, NULL);
      return rc;
    }
  }

  if (snprintf(pragma_sql, sizeof(pragma_sql), "PRAGMA user_version=%d;",
               migration->version) >= (int)sizeof(pragma_sql)) {
    cmaper_snapshot_schema_diag_setf(diag, "schema",
                                     "failed to format user_version pragma");
    sqlite3_exec(db, "ROLLBACK;", NULL, NULL, NULL);
...
void cmaper_snapshot_schema_diag_setf(cmaper_snapshot_schema_diag_t *diag,
...
cmaper_err_t
cmaper_snapshot_schema_bootstrap(sqlite3 *db,
                                 cmaper_snapshot_schema_diag_t *diag) {
