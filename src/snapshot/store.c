...
static int cmaper_snapshot_bool_to_int(bool value) {
    return value ? 1 : 0;
}

static void cmaper_snapshot_diag_set_sqlite(
...
cmaper_err_t cmaper_snapshot_store_open(
    ...
    if (!enable_persistence) {
        cmaper_log(logger, CMAPER_LOG_INFO, "snapshot/store: persistence is disabled (xml-only mode)");
        return CMAPER_OK;
    }
    ...
    if (rc != CMAPER_OK) {
        cmaper_snapshot_diag_setf(
            diag,
            "db-dir",
            "failed to ensure db directory '%s'",
            paths->db_dir
        );
        return rc;
    }
    ...
    if (rc != CMAPER_OK) {
        cmaper_snapshot_diag_setf(
            diag,
            schema_diag.field != NULL ? schema_diag.field : "schema",
            "%s",
            schema_diag.message[0] != '\0' ? schema_diag.message : "schema bootstrap failed"
        );
        sqlite3_close(db);
        return rc;
    }
    ...
    cmaper_log(logger, CMAPER_LOG_OK, "snapshot/store: ready at '%s'", store->db_path);
...
cmaper_err_t cmaper_snapshot_session_start(
    ...
    static const char *SQL =
        ...
        "  session_uid,status,target,profile,exact_ports,no_ping,timing_template,detail_workers,"
        "  service_detection,os_detection,sudo,spoof_mac_mode,spoof_mac_value,traceroute,udp_enrichment,"
        "  started_at,completed_at,failed_at,error_message,discovery_xml_path,"
        "  detail_targets_total,detail_hosts_success,detail_hosts_failed,detail_hosts_degraded"
        ") VALUES ("
        "  ?, 'running', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,"
        "  strftime('%Y-%m-%dT%H:%M:%fZ','now'), NULL, NULL, NULL, NULL, 0, 0, 0, 0"
        ")"
    ...
    if (request == NULL || request->session_uid == NULL || request->session_uid[0] == '\0'
        || request->plan == NULL) {
        return CMAPER_ERR_INVALID_ARGUMENT;
    }
