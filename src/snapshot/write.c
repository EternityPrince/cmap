...
cmaper_snapshot_write_scan_data(sqlite3 *db,
  ...
  if ((request->scan_result->discovery_xml == NULL ||
       request->scan_result->discovery_xml_size == 0) &&
  ...
  if (request->scan_result->discovery_xml != NULL &&
      request->scan_result->discovery_xml_size > 0) {
    rc = cmaper_nmap_xml_parse_memory(request->scan_result->discovery_xml,
                                      request->scan_result->discovery_xml_size,
                                      &discovery_doc, &discovery_diag);
  } else {
    rc = cmaper_nmap_xml_parse_file(request->scan_result->discovery_xml_path,
                                    &discovery_doc, &discovery_diag);
  }
  if (rc != CMAPER_OK) {
    cmaper_log(logger, CMAPER_LOG_FAIL,
               "snapshot/write: discovery xml parse failed: %s",
               discovery_diag.message[0] != '\0' ? discovery_diag.message
                                                 : "parse error");
    cmaper_nmap_xml_document_dispose(&discovery_doc);
    return CMAPER_ERR_PARSE;
  }

  rc = cmaper_snapshot_build_detail_docs(request->scan_result, &detail_docs,
                                         &detail_doc_count, logger);
  if (rc != CMAPER_OK) {
    cmaper_nmap_xml_document_dispose(&discovery_doc);
    return rc;
  }

  rc = cmaper_snapshot_build_merged_hosts(&discovery_doc, detail_docs,
                                          detail_doc_count, &merged_hosts,
                                          &merged_host_count);
  if (rc != CMAPER_OK) {
  ...
  rc = cmaper_snapshot_upsert_session_networks(db, session_id,
                                               request->plan->target);
  if (rc != CMAPER_OK) {
    goto rollback;
  }
  ...
  for (i = 0; i < merged_host_count; ++i) {
    rc = cmaper_snapshot_persist_merged_host(db, session_id, &merged_hosts[i],
                                             logger);
    if (rc != CMAPER_OK) {
      goto rollback;
    }
  ...
  cmaper_log(logger, CMAPER_LOG_OK,
