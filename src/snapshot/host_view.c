...
static bool cmaper_snapshot_streq_ci(const char *left, const char *right) {
    ...
    while (left[i] != '\0' && right[i] != '\0') {
        if (cmaper_snapshot_ascii_lower(left[i]) != cmaper_snapshot_ascii_lower(right[i])) {
            return false;
        }
        i += 1U;
    }
...
static const char *cmaper_snapshot_strcasestr_local(const char *haystack, const char *needle) {
    size_t i;
    size_t j;
    ...
    for (i = 0; haystack[i] != '\0'; ++i) {
        for (j = 0; needle[j] != '\0'; ++j) {
            ...
            if (cmaper_snapshot_ascii_lower(haystack[i + j]) != cmaper_snapshot_ascii_lower(needle[j])) {
                break;
            }
        }
        if (needle[j] == '\0') {
...
static bool cmaper_snapshot_extract_token_after_marker(
    const char *text,
    const char *marker,
    char *out,
    size_t out_cap
...
static void cmaper_snapshot_trim_cert_dn_tail(char *value) {
    ...
    for (i = 0; i < sizeof(TAIL_MARKERS) / sizeof(TAIL_MARKERS[0]); ++i) {
        const char *found = cmaper_snapshot_strcasestr_local(value, TAIL_MARKERS[i]);
...
static bool cmaper_snapshot_extract_hostname_from_script(
    const cmaper_nmap_xml_script_t *script,
    char *out,
    size_t out_cap
) {
    ...
    if (cmaper_snapshot_streq_ci(script->id, "ssl-cert")) {
        if (cmaper_snapshot_extract_token_after_marker(script->output, "commonName=", out, out_cap)) {
            cmaper_snapshot_trim_cert_dn_tail(out);
            return cmaper_snapshot_hostname_is_valid(out);
        }
        if (cmaper_snapshot_extract_token_after_marker(script->output, "CN=", out, out_cap)) {
            cmaper_snapshot_trim_cert_dn_tail(out);
            return cmaper_snapshot_hostname_is_valid(out);
        }
    }
    ...
    if (cmaper_snapshot_streq_ci(script->id, "smb-os-discovery")) {
        if (cmaper_snapshot_extract_token_after_marker(script->output, "Computer name", out, out_cap)) {
            return cmaper_snapshot_hostname_is_valid(out);
        }
    }

    if (cmaper_snapshot_streq_ci(script->id, "nbstat")) {
        if (cmaper_snapshot_extract_token_after_marker(script->output, "NetBIOS name", out, out_cap)) {
            return cmaper_snapshot_hostname_is_valid(out);
        }
    }
...
static const char *cmaper_snapshot_hostname_from_scripts(
    ...
    for (i = 0; i < host->host_script_count; ++i) {
        if (cmaper_snapshot_extract_hostname_from_script(&host->host_scripts[i], out, out_cap)) {
            return out;
        }
    }
    ...
    for (i = 0; i < host->port_count; ++i) {
        ...
        for (j = 0; j < port->script_count; ++j) {
            if (cmaper_snapshot_extract_hostname_from_script(&port->scripts[j], out, out_cap)) {
                return out;
            }
        }
    }
...
static const char *cmaper_snapshot_pick_hostname(
    ...
    value = cmaper_snapshot_hostname_from_scripts(primary, fallback, fallback_cap);
    if (value != NULL) {
        return value;
    }

    return cmaper_snapshot_hostname_from_scripts(secondary, fallback, fallback_cap);
}
