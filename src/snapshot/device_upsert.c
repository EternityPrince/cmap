...
cmaper_err_t cmaper_snapshot_insert_device(sqlite3 *db, const char *stable_key,
  ...
  if (db == NULL || stable_key == NULL || fallback_key == NULL ||
      out_device_id == NULL) {
...
cmaper_err_t cmaper_snapshot_upsert_device_ip(sqlite3 *db,
  ...
  static const char *SQL_UPSERT =
      ...
      "  device_id, ip_address, address_type, first_seen_session_id, "
      "last_seen_session_id, is_current"
      ") VALUES(?, ?, ?, ?, ?, 1) "
  ...
  if (db == NULL || device_id <= 0 || ip_address == NULL ||
      ip_address[0] == '\0') {
    return CMAPER_ERR_INVALID_ARGUMENT;
  }
