...
cmaper_snapshot_persist_merged_host(sqlite3 *db, sqlite3_int64 session_id,
  ...
  rc = cmaper_snapshot_resolve_device(
      db, session_id, host_view.ip, host_view.ip_type,
      host_view.mac != NULL ? host_view.mac->addr : NULL,
      host_view.mac != NULL ? host_view.mac->vendor : NULL, &device_id);
  if (rc != CMAPER_OK) {
    goto cleanup;
  }

  rc = cmaper_snapshot_upsert_host_observation(
      db, session_id, device_id, host_view.ip, host_view.ip_type,
      host_view.status, host_view.observation_source, host_view.hostname,
      host_view.mac != NULL ? host_view.mac->addr : NULL,
      host_view.mac != NULL ? host_view.mac->vendor : NULL,
      host_view.detail_xml_path, &host_observation_id);
  if (rc != CMAPER_OK) {
    goto cleanup;
  }

  rc = cmaper_snapshot_replace_host_children(db, host_observation_id,
                                             &host_view);
  if (rc != CMAPER_OK) {
    goto cleanup;
  }
  ...
  rc = cmaper_security_extract_from_host_pair(
      host_view.primary, host_view.secondary, &security_artifacts);
  if (rc != CMAPER_OK) {
    goto cleanup;
  }

  rc = cmaper_snapshot_security_persist_host_artifacts(
      db, host_observation_id, &security_artifacts, logger);
