...
void cmaper_history_device_change_event_init(
  ...
  cmaper_history_zero_bytes(event, sizeof(*event));
}
...
void cmaper_history_device_report_init(cmaper_history_device_report_t *report) {
  ...
  report->has_window_days = false;
  report->window_days = 0;
  report->changes_only = false;
  report->ip_addresses = NULL;
  report->ip_address_count = 0;
  report->observations = NULL;
  report->observation_count = 0;
  report->change_events = NULL;
  report->change_event_count = 0;
  report->changes_truncated = false;
}
...
void cmaper_history_device_report_dispose(
  ...
  if (report->change_events != NULL) {
    free(report->change_events);
  }
