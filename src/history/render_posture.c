...
static const char *cmaper_history_posture_risk_level(const cmaper_history_posture_report_t *report) {
    if (report == NULL) {
        return "info";
    }
    if (report->counters.findings_high_or_worse > 0U
        || (report->drift.has_previous && report->drift.findings_high_or_worse_delta > 0L)) {
        return "critical";
    }
...
void cmaper_history_render_posture(
    ...
    if (format == CMAPER_OUTPUT_FORMAT_MARKDOWN) {
        ...
        fprintf(
            stream,
            "- Management surfaces: **%zu** across **%zu** hosts\n",
            report->counters.management_surfaces_total,
            report->counters.hosts_with_management_surfaces
        );
        if (report->drift.has_previous) {
            fprintf(
                stream,
                "- Drift vs `%s`: open findings **%+ld**, high/critical **%+ld**, surfaces **%+ld**\n",
        ...
        fprintf(
            stream,
            "| Hosts with management surfaces | %zu |\n",
            report->counters.hosts_with_management_surfaces
        );
        ...
        if (report->drift.has_previous) {
            ...
            fprintf(
                stream,
                "- Findings high/critical delta: **%+ld**\n",
                report->drift.findings_high_or_worse_delta
            );
            fprintf(
                stream,
                "- Management surfaces delta: **%+ld**\n",
                report->drift.management_surfaces_total_delta
            );
            fprintf(
                stream,
                "- Hosts with management surfaces delta: **%+ld**\n",
                report->drift.hosts_with_management_surfaces_delta
            );
            fprintf(
                stream,
        ...
        } else if (report->drift.risk_increased || report->counters.management_surfaces_total > 0U) {
            fputs("1. Investigate why risk counters increased versus the previous session.\n", stream);
            fputs("2. Reduce exposed management surfaces where possible.\n", stream);
        } else {
            fputs("1. Keep this posture snapshot as baseline for future drift checks.\n", stream);
    ...
    cmaper_history_render_key_value(stream, "Session status", report->session_status);
    cmaper_history_render_key_value(stream, "Device filter", report->has_device_filter ? "enabled" : "not set");
    if (report->has_device_filter) {
    ...
    cmaper_history_render_risk(
        stream,
        use_ansi,
        cmaper_history_posture_risk_level(report),
        report->counters.findings_high_or_worse > 0U
            ? "Open high/critical findings are currently present."
            : (report->drift.risk_increased
                ? "Risk indicators increased versus the previous completed session."
                : ((report->counters.findings_open > 0U || report->counters.management_surfaces_total > 0U)
                    ? "There are open findings or management exposures to review."
                    : "No immediate high-risk signal in this posture snapshot."))
    );
    fprintf(
        stream,
        "  Open findings: %zu (high/critical: %zu)\n",
    ...
    fprintf(
        stream,
        "Counters: hosts total=%zu up=%zu devices=%zu open_tcp=%zu findings total=%zu open=%zu high=%zu surfaces=%zu hosts-with-surfaces=%zu\n",
        report->counters.hosts_total,
        report->counters.hosts_up,
        report->counters.devices_total,
        report->counters.open_tcp_ports,
    ...
    if (report->drift.has_previous) {
        ...
        cmaper_history_render_key_signed(stream, use_ansi, "  Hosts total delta", report->drift.hosts_total_delta);
        cmaper_history_render_key_signed(stream, use_ansi, "  Hosts up delta", report->drift.hosts_up_delta);
        cmaper_history_render_key_signed(stream, use_ansi, "  Devices delta", report->drift.devices_total_delta);
        cmaper_history_render_key_signed(stream, use_ansi, "  Open TCP delta", report->drift.open_tcp_ports_delta);
        cmaper_history_render_key_signed(stream, use_ansi, "  Findings total delta", report->drift.findings_total_delta);
        cmaper_history_render_key_signed(stream, use_ansi, "  Findings open delta", report->drift.findings_open_delta);
        cmaper_history_render_key_signed(
            stream,
            use_ansi,
            "  Findings high/critical delta",
            report->drift.findings_high_or_worse_delta
        );
        cmaper_history_render_key_signed(
            stream,
            use_ansi,
            "  Management surfaces delta",
            report->drift.management_surfaces_total_delta
        );
        cmaper_history_render_key_signed(
            stream,
            use_ansi,
            "  Hosts with management surfaces delta",
            report->drift.hosts_with_management_surfaces_delta
        );
