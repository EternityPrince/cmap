...
static const char *cmaper_history_findings_risk_level(size_t high_findings, size_t open_findings) {
    if (high_findings > 0U) {
        return "critical";
    }
    if (open_findings > 0U) {
...
static void cmaper_history_copy_text_range(
    char *out,
    size_t out_cap,
    const char *start,
    size_t len
...
static bool cmaper_history_script_row_next(
    const char **cursor,
    char *line,
    size_t line_cap
) {
...
static void cmaper_history_script_row_decode(
    const char *row,
    char *script,
    size_t script_cap,
    char *target,
    ...
    cmaper_history_copy_text_range(script, script_cap, row, (size_t) (tab1 - row));
    cmaper_history_copy_text_range(target, target_cap, tab1 + 1, (size_t) (tab2 - (tab1 + 1)));
    if (result != NULL && result_cap > 0U) {
...
static const char *cmaper_history_session_color(
    bool use_ansi,
...
static void cmaper_history_render_host_header_terminal(
    FILE *stream,
    bool use_ansi,
    ...
    fprintf(
        ...
        reset,
        title,
        cmaper_history_value_or_dash(device_id),
        reset,
        index,
        total
    );
...
static void cmaper_history_render_host_kv_terminal(
    FILE *stream,
    bool use_ansi,
    const char *label,
    const char *value
) {
    const char *label_color = cmaper_history_session_color(use_ansi, "\033[1;34m");
    const char *value_color = cmaper_history_session_color(use_ansi, "\033[0;37m");
    const char *reset = cmaper_history_session_reset(use_ansi);
    ...
    fprintf(
        ...
        reset,
        value_color,
        cmaper_history_value_or_dash(value),
        reset
    );
}

static void cmaper_history_render_detail_list_terminal(
    FILE *stream,
    bool use_ansi,
    const char *title,
    const char *value
    ...
    const char *cursor;
    size_t emitted = 0U;
    const char *title_color = cmaper_history_session_color(use_ansi, "\033[1;33m");
...
static void cmaper_history_markdown_escape_cell(
    char *out,
    size_t out_cap,
    const char *value
) {
...
static void cmaper_history_render_script_results_markdown(
    ...
    while (cmaper_history_script_row_next(&cursor, line, sizeof(line))) {
        ...
        cmaper_history_script_row_decode(
            line,
            script,
            sizeof(script),
            target,
...
static void cmaper_history_render_script_results_terminal(
    ...
    while (cmaper_history_script_row_next(&cursor, line, sizeof(line))) {
        ...
        fprintf(
            ...
            "        %s%s%s\n",
            item_color,
            cmaper_history_value_or_dash(script),
            cmaper_history_value_or_dash(target),
            reset,
            item_color,
            cmaper_history_value_or_dash(result),
            reset
        );
    }
}
...
void cmaper_history_render_sessions(
    ...
    if (format == CMAPER_OUTPUT_FORMAT_MARKDOWN) {
        ...
        fprintf(
            stream,
            "- **Risk:** **%s** (highest in shown sessions: open findings **%zu**, high/critical **%zu**)\n",
        ...
        fputs(
            "\n## Details\n\n| Session | Status | Started | Hosts | Open findings (high) | Surfaces |\n"
            "|---|---|---|---:|---:|---:|\n",
            stream
        );
        for (i = 0; i < shown_count; ++i) {
        ...
        cmaper_history_render_truncated_note(stream, true, shown_count, report->count, "sessions");
        return;
    }
    ...
    fprintf(stream, "  Requested limit: %d\n", report->limit);
    cmaper_history_render_key_size(stream, "Sessions available", report->total_sessions);
...

    {
        size_t max_open = 0U;
        size_t max_high = 0U;
        const char *latest_session = shown_count > 0U ? report->items[0].session_id : "-";
        ...
        cmaper_history_render_risk(
            ...
            max_high > 0U
                ...
                : (max_open > 0U
                    ? "Open findings are present in shown sessions."
        ...
        for (i = 0; i < shown_count; ++i) {
            ...
            fprintf(
                stream,
                "  %s  status=%s  hosts=%zu  open findings=%zu (high=%zu)  surfaces=%zu\n",
                row->session_id,
                row->status,
                row->host_count,
                row->findings_open,
    ...
    cmaper_history_render_truncated_note(stream, false, shown_count, report->count, "sessions");
}
...
void cmaper_history_render_session(
    ...
    if (format == CMAPER_OUTPUT_FORMAT_MARKDOWN) {
        ...
        if (strcmp(report->summary.profile, "low") == 0) {
            fputs(
                "- Script pipeline: low profile keeps service detection/scripts disabled unless `--service-detection` is enabled.\n",
                stream
            );
        }

        fputs(
            "\n## Details\n\n| Device | IP | Host status | Hostname | Open TCP | Open findings (high) | Surfaces |\n"
            "|---|---|---|---|---:|---:|---:|\n",
            stream
        );
        for (i = 0; i < shown_count; ++i) {
        ...
        if (view == CMAPER_OUTPUT_VIEW_FULL && shown_count > 0U) {
            ...
            for (i = 0; i < shown_count; ++i) {
                ...
                if (host->script_result_count > 0U) {
                    ...
                    fputs(
                        "  - Scripts used / Results: `none` (low profile default keeps service detection/scripts off)\n",
                        stream
                    );
                } else {
                ...
                if (host->script_signal_count > 0U) {
                    fprintf(
                        stream,
                        "  - Script-derived signals (%zu): `%s`\n",
                        host->script_signal_count,
                        host->script_signals
                    );
                } else if (strcmp(report->summary.profile, "low") == 0) {
                    fputs(
                        "  - Script-derived signals: `none` (low profile default keeps service detection/scripts off)\n",
                        stream
                    );
                } else {
        ...
        cmaper_history_render_truncated_note(stream, true, shown_count, report->host_count, "hosts");
        return;
    }
    ...
    cmaper_history_render_heading(stream, use_ansi, "Session");
    cmaper_history_render_section(stream, use_ansi, "Context");
    cmaper_history_render_key_value(stream, "Session", report->summary.session_id);
    ...
    cmaper_history_render_key_value(stream, "Started", report->summary.started_at);
    cmaper_history_render_key_value(stream, "Completed", report->summary.completed_at);

    cmaper_history_render_section(stream, use_ansi, "Key Takeaways");
    cmaper_history_render_risk(
        stream,
        use_ansi,
        cmaper_history_findings_risk_level(
            report->summary.findings_high_or_worse,
            report->summary.findings_open
        ),
        report->summary.findings_high_or_worse > 0U
    ...
    cmaper_history_render_key_size(stream, "Hosts observed", report->summary.host_count);
    fprintf(
        stream,
        "  Open findings: %zu (high/critical: %zu)\n",
        report->summary.findings_open,
        report->summary.findings_high_or_worse
    );
    cmaper_history_render_key_size(stream, "Management surfaces", report->summary.management_surfaces_total);
    if (strcmp(report->summary.profile, "low") == 0) {
        cmaper_history_render_key_value(
            stream,
            "Script pipeline",
            "low profile default keeps service detection/scripts disabled (enable with --service-detection)"
        );
    }
    ...
    for (i = 0; i < shown_count; ++i) {
        ...
        cmaper_history_render_host_kv_terminal(stream, use_ansi, "IP", host->primary_ip);
        cmaper_history_render_host_kv_terminal(stream, use_ansi, "Host status", host->status);
        cmaper_history_render_host_kv_terminal(stream, use_ansi, "Hostname", host->hostname);
        snprintf(
            summary,
            sizeof(summary),
        ...
        cmaper_history_render_host_kv_terminal(stream, use_ansi, "Summary", summary);
    ...
    cmaper_history_render_truncated_note(stream, false, shown_count, report->host_count, "hosts");
}
