...
static const char *cmaper_history_timeline_risk_level(
...
void cmaper_history_render_timeline(
    ...
    if (format == CMAPER_OUTPUT_FORMAT_MARKDOWN) {
        ...
        if (shown_count == 0U) {
            ...
            const cmaper_history_timeline_row_t *oldest_row = &report->items[shown_count - 1U];
            long open_delta = (long) anchor_row->findings_open - (long) oldest_row->findings_open;
            ...
            fprintf(
                stream,
                "- **Risk:** **%s** for anchor session (open findings **%zu**, high/critical **%zu**)\n",
                anchor_row->findings_high_or_worse > 0U
                    ? "CRITICAL"
                    : (anchor_row->findings_open > 0U ? "WARN" : "OK"),
                anchor_row->findings_open,
                anchor_row->findings_high_or_worse
            );
            fprintf(
                stream,
                "- Drift from oldest shown row to anchor: open findings **%+ld**, high/critical **%+ld**, surfaces **%+ld**\n",
                open_delta,
                high_delta,
                surfaces_delta
            );
        ...
        fputs(
            "\n## Details\n\n| Session | Session status | Hosts | Open findings (high) | Surfaces | Device present | Device IP |\n"
            "|---|---|---:|---:|---:|---|---|\n",
            stream
        );
        for (i = 0; i < shown_count; ++i) {
        ...
        cmaper_history_render_truncated_note(stream, true, shown_count, report->count, "timeline rows");
        return;
    }
    ...
    cmaper_history_render_key_value(stream, "Anchor session", report->anchor_session_id);
    ...
    if (shown_count == 0U) {
        ...
        cmaper_history_render_risk(
            stream,
            use_ansi,
            cmaper_history_timeline_risk_level(
                anchor_row->findings_high_or_worse,
                anchor_row->findings_open
            ),
            anchor_row->findings_high_or_worse > 0U
        ...
        cmaper_history_render_key_signed(
            stream,
            use_ansi,
            "Open findings drift (anchor - oldest shown)",
            (long) anchor_row->findings_open - (long) oldest_row->findings_open
        );
        cmaper_history_render_key_signed(
            stream,
            use_ansi,
            "High/critical drift (anchor - oldest shown)",
            (long) anchor_row->findings_high_or_worse - (long) oldest_row->findings_high_or_worse
        );
        cmaper_history_render_key_signed(
            stream,
            use_ansi,
            "Management surfaces drift (anchor - oldest shown)",
            (long) anchor_row->management_surfaces - (long) oldest_row->management_surfaces
        );
        if (report->has_device_filter) {
            fprintf(
                stream,
                "  Anchor device presence: %s (ip=%s)\n",
                anchor_row->device_present ? "yes" : "no",
                anchor_row->device_ip
            );
        }
    ...
    for (i = 0; i < shown_count; ++i) {
        ...
        if (report->has_device_filter) {
            fprintf(
                stream,
                "    device present=%s ip=%s host-status=%s\n",
    ...
    cmaper_history_render_truncated_note(stream, false, shown_count, report->count, "timeline rows");
}
