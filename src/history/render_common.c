...
static bool cmaper_history_render_level_is(const char *level,
                                           const char *expected) {
...
static void cmaper_history_render_badge(FILE *stream, bool use_ansi,
                                        const char *level, const char *text) {
  if (stream == NULL || text == NULL) {
    return;
  }

  if (use_ansi) {
    fprintf(stream, "%s[%s]\033[0m", cmaper_history_render_level_color(level),
            text);
  } else {
...
void cmaper_history_render_risk(FILE *stream, bool use_ansi, const char *level,
  ...
  fputs("  Risk: ", stream);
  cmaper_history_render_badge(stream, use_ansi, level,
                              cmaper_history_render_level_text(level));
  fprintf(stream, " %s\n",
          summary != NULL && summary[0] != '\0' ? summary
...
void cmaper_history_render_truncated_note(FILE *stream, bool markdown,
  ...
  const char *item_label =
      subject != NULL && subject[0] != '\0' ? subject : "rows";
  ...
  if (markdown) {
    fprintf(stream,
            "\n_Notes: showing first %zu of %zu %s (use `--view full` for the "
            "full report)._\n",
            shown, total, item_label);
  } else {
    fputs("\nNotes\n", stream);
    fprintf(stream,
            "  Showing first %zu of %zu %s. Use --view full for the complete "
            "report.\n",
            shown, total, item_label);
  }
}
...
void cmaper_history_render_reason_mask_json(FILE *stream, unsigned int mask) {
  ...
  for (i = 0; i < sizeof(REASONS) / sizeof(REASONS[0]); ++i) {
    ...
    cmaper_history_json_string(stream,
                               cmaper_history_host_reason_name(REASONS[i]));
    first = false;
  }
  fputc(']', stream);
}
