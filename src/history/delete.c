...
static cmaper_err_t cmaper_history_delete_prepare(sqlite3 *db, const char *sql,
...
static cmaper_err_t cmaper_history_delete_bind_int64(sqlite3_stmt *stmt,
                                                     int index,
                                                     sqlite3_int64 value) {
  if (stmt == NULL) {
    return CMAPER_ERR_INVALID_ARGUMENT;
...
static cmaper_err_t cmaper_history_delete_count(sqlite3 *db, const char *sql,
                                                size_t *out_count) {
...
static cmaper_err_t cmaper_history_delete_open_rw(const char *db_path,
                                                  sqlite3 **out_db,
...
static cmaper_err_t
cmaper_history_delete_rebuild_metadata(sqlite3 *db,
  ...
  if (db == NULL || out_orphan_devices_deleted == NULL ||
      out_orphan_networks_deleted == NULL) {
    return CMAPER_ERR_INVALID_ARGUMENT;
  }
...
cmaper_history_delete_session(const char *db_path, const char *session_token,
  ...
  if (db_path == NULL || session_token == NULL || session_token[0] == '\0' ||
      report == NULL) {
    return CMAPER_ERR_INVALID_ARGUMENT;
  }
  ...
  (void)snprintf(report->session_id, sizeof(report->session_id), "%s",
  ...
  rc = cmaper_history_delete_rebuild_metadata(
      db, &report->orphan_devices_deleted, &report->orphan_networks_deleted);
  if (rc != CMAPER_OK) {
    (void)cmaper_history_delete_exec(db, "ROLLBACK;");
    cmaper_history_delete_close_db(&db);
    return rc;
...
cmaper_history_delete_all_sessions(const char *db_path,
  ...
  rc = cmaper_history_delete_count(db, "SELECT COUNT(*) FROM scan_sessions;",
