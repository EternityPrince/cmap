...
static bool cmaper_history_mode_is_delete(cmaper_cli_mode_t mode) {
  return mode == CMAPER_CLI_MODE_DELETE_SESSION ||
         mode == CMAPER_CLI_MODE_DELETE_ALL_SESSIONS;
}
...
cmaper_history_service_run(const cmaper_history_service_request_t *request) {
  ...
  report_stream =
      request->report_stream != NULL ? request->report_stream : stdout;
  ...
  rc = cmaper_output_sink_open(&sink, &request->config->output, report_stream,
                               request->logger);
  if (rc != CMAPER_OK) {
    return rc;
  }
  ...
  if (cmaper_history_mode_is_delete(request->config->mode)) {
    ...
    case CMAPER_CLI_MODE_DELETE_SESSION:
      rc = cmaper_history_run_delete_session(request->config, request->paths,
                                             sink.stream);
      break;
    case CMAPER_CLI_MODE_DELETE_ALL_SESSIONS:
      rc = cmaper_history_run_delete_all(request->config, request->paths,
                                         sink.stream);
      break;
    ...
    sink_rc = cmaper_output_sink_finalize(&sink, &request->config->output,
                                          report_stream, request->logger);
    ...
    if (rc == CMAPER_OK) {
      ...
      cmaper_log(request->logger, CMAPER_LOG_FAIL,
  ...
  rc =
      cmaper_history_query_open_db(request->paths->db_path, &db, &db_available);
  if (rc != CMAPER_OK) {
    (void)cmaper_output_sink_finalize(&sink, &request->config->output,
                                      report_stream, request->logger);
    return rc;
  }

  cmaper_log(request->logger, CMAPER_LOG_PHASE,
  ...
  switch (request->config->mode) {
  case CMAPER_CLI_MODE_SESSIONS:
    rc = cmaper_history_run_sessions(db, db_available, request->config,
                                     sink.stream, &render_options);
    break;
  case CMAPER_CLI_MODE_SESSION:
    rc = cmaper_history_run_session_detail(db, db_available, request->config,
                                           sink.stream, &render_options);
    break;
  case CMAPER_CLI_MODE_DIFF:
    rc = cmaper_history_run_diff(db, db_available, request->config, sink.stream,
  ...
  case CMAPER_CLI_MODE_DIFF_GLOBAL:
    ...
    break;
  case CMAPER_CLI_MODE_TIMELINE:
    rc = cmaper_history_run_timeline(db, db_available, request->config,
                                     sink.stream, &render_options);
    break;
  case CMAPER_CLI_MODE_DEVICES:
    rc = cmaper_history_run_devices(db, db_available, request->config,
                                    sink.stream, &render_options);
    break;
  case CMAPER_CLI_MODE_DEVICE:
    rc = cmaper_history_run_device(db, db_available, request->config,
                                   sink.stream, &render_options);
    break;
  case CMAPER_CLI_MODE_POSTURE:
    rc = cmaper_history_run_posture(db, db_available, request->config,
                                    sink.stream, &render_options);
    break;
  case CMAPER_CLI_MODE_SCAN:
  case CMAPER_CLI_MODE_DELETE_SESSION:
  ...
  if (rc == CMAPER_OK) {
    ...
    cmaper_log(request->logger, CMAPER_LOG_FAIL,
               "history/service: mode '%s' failed (%s)",
               cmaper_cli_mode_name(request->config->mode), cmaper_err_str(rc));
  }
