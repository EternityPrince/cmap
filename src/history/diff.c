...
static void cmaper_history_copy_string(char *out, size_t out_cap,
                                       const char *value) {
...
static void
cmaper_history_make_host_key(const cmaper_history_host_snapshot_t *host,
  ...
  cmaper_history_normalize_mac(host->mac_address, normalized_mac,
...
static bool
cmaper_history_host_mac_equal(const cmaper_history_host_snapshot_t *left,
  ...
  cmaper_history_normalize_mac(right->mac_address, right_mac,
                               sizeof(right_mac));
...
static bool
cmaper_history_port_equal(const cmaper_history_port_signal_t *left,
...
static bool
cmaper_history_surface_equal(const cmaper_history_surface_signal_t *left,
...
static bool
cmaper_history_finding_is_open(const cmaper_history_finding_signal_t *finding) {
  if (finding == NULL) {
    return false;
  }
...
static bool cmaper_history_finding_is_high_or_worse(
    const cmaper_history_finding_signal_t *finding) {
  if (finding == NULL) {
    return false;
  }
...
static bool cmaper_history_surface_is_risky(
    const cmaper_history_surface_signal_t *surface) {
  static const char *RISKY_TYPES[] = {
      "ssh",        "telnet",         "rdp", "winrm-http", "winrm-https",
      "docker-api", "docker-api-tls", "smb", "vnc",        "k8s-api"};
  size_t i;
...
static size_t cmaper_history_count_open_high_findings(
  ...
  for (i = 0; i < host->finding_count; ++i) {
    ...
    if (cmaper_history_finding_is_open(finding) &&
        cmaper_history_finding_is_high_or_worse(finding)) {
      count += 1U;
    }
  }
  return count;
...
static size_t cmaper_history_count_fingerprints_added(
  ...
  for (i = 0; i < to_host->fingerprint_count; ++i) {
    ...
    for (j = 0; j < from_host->fingerprint_count; ++j) {
      if (cmaper_history_fingerprint_equal(&to_host->fingerprints[i],
                                           &from_host->fingerprints[j])) {
        found = true;
        break;
      }
    }
...
static void cmaper_history_count_finding_changes(
    const cmaper_history_host_snapshot_t *from_host,
    const cmaper_history_host_snapshot_t *to_host, size_t *out_opened,
    size_t *out_resolved, size_t *out_high_opened) {
  size_t opened = 0;
  size_t resolved = 0;
  ...
  for (i = 0; i < from_host->finding_count; ++i) {
    const cmaper_history_finding_signal_t *from_finding =
        &from_host->findings[i];
...
static void cmaper_history_count_surface_changes(
    const cmaper_history_host_snapshot_t *from_host,
    const cmaper_history_host_snapshot_t *to_host, size_t *out_added,
    size_t *out_removed, size_t *out_risky_added) {
  size_t added = 0;
  size_t removed = 0;
  ...
  for (i = 0; i < to_host->surface_count; ++i) {
    ...
    for (j = 0; j < from_host->surface_count; ++j) {
      if (cmaper_history_surface_equal(&to_host->surfaces[i],
                                       &from_host->surfaces[j])) {
        found = true;
        break;
      }
    }
  ...
  for (i = 0; i < from_host->surface_count; ++i) {
    ...
    for (j = 0; j < to_host->surface_count; ++j) {
      if (cmaper_history_surface_equal(&from_host->surfaces[i],
                                       &to_host->surfaces[j])) {
        found = true;
        break;
      }
...
static int cmaper_history_find_to_match_by_mac(
    const cmaper_history_host_snapshot_t *from_host,
    const cmaper_history_host_snapshot_t *to_hosts, const bool *to_matched,
    size_t to_count) {
  int fallback = -1;
  size_t i;
  ...
  for (i = 0; i < to_count; ++i) {
    ...
    if (cmaper_history_compare_ip(from_host->primary_ip,
                                  to_hosts[i].primary_ip) == 0) {
      return (int)i;
    }
    if (fallback < 0) {
      fallback = (int)i;
...
static cmaper_err_t
cmaper_history_append_changed_host(cmaper_history_diff_report_t *report,
  ...
  next = (cmaper_history_changed_host_t *)realloc(
      report->changed_hosts, (report->changed_host_count + 1U) *
                                 sizeof(cmaper_history_changed_host_t));
  if (next == NULL) {
...
cmaper_err_t cmaper_history_diff_build(
  ...
  for (i = 0; i < from_host_count; ++i) {
    ...
    to_index = cmaper_history_find_to_match_by_mac(from_host, to_hosts,
                                                   to_matched, to_host_count);
    if (to_index >= 0) {
      matched_by_mac = true;
    } else {
      to_index = cmaper_history_find_to_match_by_ip(from_host, to_hosts,
                                                    to_matched, to_host_count);
    ...
    if (to_index < 0) {
      ...
      cmaper_history_make_host_key(from_host, row.host_key,
                                   sizeof(row.host_key));
      cmaper_history_copy_string(row.match_strategy, sizeof(row.match_strategy),
                                 "none");
      cmaper_history_copy_string(row.from_ip, sizeof(row.from_ip),
                                 from_host->primary_ip);
      cmaper_history_copy_string(row.from_status, sizeof(row.from_status),
                                 from_host->status);
      cmaper_history_copy_string(row.from_hostname, sizeof(row.from_hostname),
                                 from_host->hostname);
      cmaper_history_copy_string(row.mac_address, sizeof(row.mac_address),
                                 from_host->mac_address);
      row.reason_mask |= CMAPER_HISTORY_HOST_REASON_REMOVED;
      row.from_high_findings_open =
          cmaper_history_count_open_high_findings(from_host);
...

    {
      ...
      cmaper_history_changed_host_init(&row);
      cmaper_history_make_host_key(matched_by_mac ? to_host : from_host,
                                   row.host_key, sizeof(row.host_key));
      cmaper_history_copy_string(row.match_strategy, sizeof(row.match_strategy),
                                 matched_by_mac ? "mac" : "ip");
      cmaper_history_copy_string(row.from_ip, sizeof(row.from_ip),
                                 from_host->primary_ip);
      cmaper_history_copy_string(row.to_ip, sizeof(row.to_ip),
                                 to_host->primary_ip);
      cmaper_history_copy_string(row.from_status, sizeof(row.from_status),
                                 from_host->status);
      cmaper_history_copy_string(row.to_status, sizeof(row.to_status),
                                 to_host->status);
      cmaper_history_copy_string(row.from_hostname, sizeof(row.from_hostname),
                                 from_host->hostname);
      cmaper_history_copy_string(row.to_hostname, sizeof(row.to_hostname),
                                 to_host->hostname);
      cmaper_history_copy_string(row.mac_address, sizeof(row.mac_address),
                                 to_host->mac_address);
      row.from_high_findings_open =
          cmaper_history_count_open_high_findings(from_host);
      row.to_high_findings_open =
          cmaper_history_count_open_high_findings(to_host);

      if (cmaper_history_compare_ip(from_host->primary_ip,
                                    to_host->primary_ip) != 0) {
      ...
      row.ports_removed =
          cmaper_history_count_ports_removed(from_host, to_host);
      if (row.ports_added > 0 || row.ports_removed > 0) {
        row.reason_mask |= CMAPER_HISTORY_HOST_REASON_PORTS_CHANGED;
      }

      row.fingerprints_added =
          cmaper_history_count_fingerprints_added(from_host, to_host);
      row.fingerprints_removed =
          cmaper_history_count_fingerprints_removed(from_host, to_host);
      ...
      cmaper_history_count_finding_changes(
          from_host, to_host, &row.findings_opened, &row.findings_resolved,
      ...
      cmaper_history_count_surface_changes(
          from_host, to_host, &row.management_added, &row.management_removed,
