...
sqlite3_int64 cmaper_history_resolve_filter_device_id(sqlite3 *db,
  ...
  if (cmaper_history_query_resolve_device(db, device_token, &device_id) !=
      CMAPER_OK) {
    return MISSING_SENTINEL;
  }
...
cmaper_err_t cmaper_history_run_sessions(
  ...
  if (db_available) {
    rc = cmaper_history_query_sessions(
        db, cmaper_history_limit_for_mode(config), &report);
    if (rc != CMAPER_OK) {
...
cmaper_err_t cmaper_history_run_session_detail(
  ...
  if (db_available) {
    ...
    report.db_available = false;
    cmaper_history_copy_string(report.summary.session_id,
...
cmaper_err_t cmaper_history_run_devices(
  ...
  if (db_available) {
    ...
    rc = cmaper_history_query_devices(
        db, &session_ref, cmaper_history_limit_for_mode(config), &report);
    if (rc != CMAPER_OK) {
      cmaper_history_devices_report_dispose(&report);
      return rc;
    }
...
cmaper_err_t cmaper_history_run_device(
  ...
  cmaper_history_session_ref_t session_ref;
  cmaper_history_device_query_options_t query_options;
  sqlite3_int64 device_id = 0;
  cmaper_err_t rc = CMAPER_OK;

  cmaper_history_device_report_init(&report);
  cmaper_history_session_ref_init(&session_ref);
  query_options.has_window_days = config->history.has_window_days;
  query_options.window_days = config->history.window_days;
  query_options.changes_only = config->history.changes_only;
  ...
  if (db_available) {
    ...
    rc = cmaper_history_query_resolve_device(db, config->history.device_id,
                                             &device_id);
    if (rc != CMAPER_OK) {
      cmaper_history_device_report_dispose(&report);
      return rc;
    ...
    if (device_id > 0) {
      rc = cmaper_history_query_device(db, &session_ref, device_id,
                                       &query_options, &report);
      if (rc != CMAPER_OK) {
        cmaper_history_device_report_dispose(&report);
        return rc;
      }
...
cmaper_err_t cmaper_history_run_timeline(
  ...
  if (db_available) {
    ...
    rc = cmaper_history_query_timeline(db, &anchor_ref, device_id,
    ...
    if (report.anchor_session_id[0] == '\0') {
      cmaper_history_copy_string(report.anchor_session_id,
