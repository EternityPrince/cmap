...
static cmaper_err_t
cmaper_history_append_alert(cmaper_history_alert_t **items, size_t *count,
  ...
  if (items == NULL || count == NULL || severity == NULL || code == NULL ||
      title == NULL) {
    return CMAPER_ERR_INVALID_ARGUMENT;
  }

  next = (cmaper_history_alert_t *)realloc(
      *items, (*count + 1U) * sizeof(cmaper_history_alert_t));
  if (next == NULL) {
...
static void cmaper_history_clear_alerts(cmaper_history_alert_t **items,
                                        size_t *count) {
...
cmaper_history_alerts_build_for_diff(cmaper_history_diff_report_t *report) {
  ...
  if (report->summary.findings_high_opened > 0) {
    ...
    rc = cmaper_history_append_alert(
        &report->alerts, &report->alert_count, "critical", "new-high-findings",
        "New high-severity findings detected", detail, NULL);
    if (rc != CMAPER_OK) {
      return rc;
    }
  }
  ...
  if (report->summary.findings_opened > report->summary.findings_resolved &&
    ...
    rc = cmaper_history_append_alert(&report->alerts, &report->alert_count,
                                     "warn", "finding-drift-up",
                                     "Security drift increased", detail, NULL);
    if (rc != CMAPER_OK) {
      return rc;
    }
  }
  ...
  for (i = 0; i < report->changed_host_count; ++i) {
    ...
    if (host->findings_high_opened > 0) {
      char detail[CMAPER_HISTORY_DETAIL_CAP];
      (void)snprintf(detail, sizeof(detail),
                     "%s opened %zu high/critical findings", host->host_key,
                     host->findings_high_opened);
      rc = cmaper_history_append_alert(&report->alerts, &report->alert_count,
                                       "critical", "host-high-findings-opened",
                                       "Host introduced high-risk findings",
                                       detail, host->host_key);
      if (rc != CMAPER_OK) {
        return rc;
      }
    }
    ...
    if (host->risky_surfaces_added > 0) {
      char detail[CMAPER_HISTORY_DETAIL_CAP];
      (void)snprintf(detail, sizeof(detail),
                     "%s added %zu risky management surfaces", host->host_key,
                     host->risky_surfaces_added);
      rc = cmaper_history_append_alert(&report->alerts, &report->alert_count,
                                       "high", "host-risky-surface-added",
    ...
    if (cmaper_history_host_reason_has(host->reason_mask,
      ...
      (void)snprintf(
          ...
          host->host_key, host->to_high_findings_open);
      rc = cmaper_history_append_alert(
          &report->alerts, &report->alert_count, "high", "moved-risky-host",
          "Risky host changed IP", detail, host->host_key);
      if (rc != CMAPER_OK) {
        return rc;
      }
    }
    ...
    if (cmaper_history_host_reason_has(host->reason_mask,
      ...
      char detail[CMAPER_HISTORY_DETAIL_CAP];
      (void)snprintf(detail, sizeof(detail),
                     "%s disappeared while previously carrying %zu high-risk "
                     "open findings",
                     host->host_key, host->from_high_findings_open);
      rc = cmaper_history_append_alert(
          &report->alerts, &report->alert_count, "warn", "removed-risky-host",
          "Previously risky host disappeared", detail, host->host_key);
      if (rc != CMAPER_OK) {
        return rc;
      }
    }
...
cmaper_err_t cmaper_history_alerts_build_for_posture(
  ...
  if (report->counters.findings_high_or_worse > 0) {
    ...
    rc = cmaper_history_append_alert(
        &report->alerts, &report->alert_count, "high", "posture-high-findings",
        "High-risk findings remain open", detail, NULL);
    if (rc != CMAPER_OK) {
      return rc;
    }
  }
  ...
  if (report->counters.management_surfaces_total > 0) {
    ...
    rc = cmaper_history_append_alert(&report->alerts, &report->alert_count,
                                     "warn", "posture-management-surface",
                                     "Management surfaces are exposed", detail,
                                     NULL);
    if (rc != CMAPER_OK) {
      return rc;
    }
  ...
  if (report->drift.has_previous &&
    ...
    rc = cmaper_history_append_alert(&report->alerts, &report->alert_count,
                                     "high", "posture-drift-high-findings",
                                     "High-risk drift increased", detail, NULL);
    if (rc != CMAPER_OK) {
      return rc;
    }
  }
  ...
  if (report->drift.risk_increased) {
    rc = cmaper_history_append_alert(
        &report->alerts, &report->alert_count, "warn", "posture-risk-increased",
        "Overall posture drift indicates higher risk",
        "At least one high-signal risk counter increased compared to previous "
        "completed session",
        NULL);
    if (rc != CMAPER_OK) {
      return rc;
