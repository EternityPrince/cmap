...
cmaper_history_load_ports(sqlite3 *db, sqlite3_int64 host_observation_id,
  ...
  static const char *SQL = "SELECT p.protocol, p.port_number "
  ...
  while ((step_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
    ...
    cmaper_history_copy_column_text(row->protocol, sizeof(row->protocol), stmt,
                                    0);
...
cmaper_history_load_fingerprints(sqlite3 *db, sqlite3_int64 host_observation_id,
  ...
  static const char *SQL =
      "SELECT x.kind, x.fingerprint, COALESCE(x.protocol,''), "
      "COALESCE(x.port_number,0) "
      "FROM ("
      "  SELECT 'tls' AS kind, tf.fingerprint AS fingerprint, p.protocol AS "
      "protocol, p.port_number AS port_number "
      ...
      "ORDER BY x.kind ASC, x.protocol ASC, x.port_number ASC, x.fingerprint "
      "ASC;";
  sqlite3_stmt *stmt = NULL;
  cmaper_history_buffer_t rows;
  cmaper_err_t rc;
  ...
  rc = cmaper_history_buffer_init(&rows,
                                  sizeof(cmaper_history_fingerprint_signal_t));
  if (rc != CMAPER_OK) {
    return rc;
  }
  rc = cmaper_history_prepare(db, SQL, &stmt);
  ...
  while ((step_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
    cmaper_history_fingerprint_signal_t *row =
        (cmaper_history_fingerprint_signal_t *)cmaper_history_buffer_push(
            &rows);
    if (row == NULL) {
      rc = CMAPER_ERR_OOM;
      goto cleanup;
...
cmaper_history_load_script_results(sqlite3 *db,
  ...
  static const char *SQL =
      ...
      "ORDER BY sr.script_id ASC, p.protocol ASC, p.port_number ASC, sr.id "
      "ASC;";
  sqlite3_stmt *stmt = NULL;
  cmaper_history_buffer_t rows;
  cmaper_err_t rc;
  ...
  rc = cmaper_history_buffer_init(
      &rows, sizeof(cmaper_history_script_result_signal_t));
  if (rc != CMAPER_OK) {
    return rc;
  }
  rc = cmaper_history_prepare(db, SQL, &stmt);
  ...
  while ((step_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
    cmaper_history_script_result_signal_t *row =
        (cmaper_history_script_result_signal_t *)cmaper_history_buffer_push(
            &rows);
    if (row == NULL) {
      rc = CMAPER_ERR_OOM;
      goto cleanup;
    }
    cmaper_history_copy_column_text(row->script_id, sizeof(row->script_id),
                                    stmt, 0);
  ...
  snapshot->script_results =
      (cmaper_history_script_result_signal_t *)rows.items;
...
cmaper_history_load_findings(sqlite3 *db, sqlite3_int64 host_observation_id,
                             cmaper_history_host_snapshot_t *snapshot) {
  static const char *SQL =
      "SELECT COALESCE(vf.finding_key,''), COALESCE(vf.severity,''), "
      "COALESCE(vf.state,''), "
      "       COALESCE(vf.title,''), COALESCE(p.protocol,''), "
      "COALESCE(p.port_number,0) "
  ...
  rc = cmaper_history_buffer_init(&rows,
                                  sizeof(cmaper_history_finding_signal_t));
  if (rc != CMAPER_OK) {
    return rc;
  }
  rc = cmaper_history_prepare(db, SQL, &stmt);
  ...
  while ((step_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
    ...
    cmaper_history_copy_column_text(row->severity, sizeof(row->severity), stmt,
                                    1);
...
cmaper_history_load_surfaces(sqlite3 *db, sqlite3_int64 host_observation_id,
  ...
  rc = cmaper_history_buffer_init(&rows,
                                  sizeof(cmaper_history_surface_signal_t));
  if (rc != CMAPER_OK) {
    return rc;
  }
  rc = cmaper_history_prepare(db, SQL, &stmt);
