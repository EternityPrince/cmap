...
static bool cmaper_history_token_boundary(char ch) {
  return ch == '\0' || isspace((unsigned char)ch) || ch == '_' || ch == '-' ||
         ch == ':' || ch == '.' || ch == ',' || ch == ';' || ch == '/' ||
         ch == '\\';
}
...
void cmaper_history_normalize_mac(const char *value, char *out,
                                  size_t out_cap) {
  size_t i;
  size_t out_len = 0;
...
bool cmaper_history_fuzzy_contains(const char *haystack, const char *needle) {
  ...
  cmaper_history_make_fuzzy_key(haystack, normalized_haystack,
                                sizeof(normalized_haystack));
  cmaper_history_make_fuzzy_key(needle, normalized_needle,
                                sizeof(normalized_needle));
