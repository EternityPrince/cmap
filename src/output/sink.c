...
static cmaper_err_t cmaper_output_sink_read_all(FILE *stream, char **out_data, size_t *out_size) {
    long size_long;
    size_t size;
    char *data;
...
cmaper_err_t cmaper_output_sink_open(
    ...
    case CMAPER_OUTPUT_TARGET_FILE:
        ...
        cmaper_log(logger, CMAPER_LOG_INFO, "output: writing report to '%s'", options->target_path);
        return CMAPER_OK;
...
cmaper_err_t cmaper_output_sink_finalize(
    ...
    if (sink->capture_for_clipboard) {
        char *data = NULL;
        size_t data_size = 0;
        cmaper_err_t read_rc = cmaper_output_sink_read_all(sink->stream, &data, &data_size);
        if (read_rc != CMAPER_OK) {
            ...
            if (clip_rc == CMAPER_OK) {
                ...
                cmaper_log(logger, CMAPER_LOG_WARN, "output: clipboard copy failed, printing report instead");
                if (data_size > 0) {
