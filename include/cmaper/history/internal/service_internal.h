...
static inline void cmaper_history_copy_string(char *out, size_t out_cap, const char *value) {
...
cmaper_err_t cmaper_history_run_delete_session(
    const cmaper_cli_config_t *config,
    const cmaper_runtime_paths_t *paths,
    FILE *report_stream
);

cmaper_err_t cmaper_history_run_delete_all(
    const cmaper_cli_config_t *config,
    const cmaper_runtime_paths_t *paths,
    FILE *report_stream
);
...
cmaper_err_t cmaper_history_run_diff(
    sqlite3 *db,
    bool db_available,
    const cmaper_cli_config_t *config,
    FILE *report_stream,
